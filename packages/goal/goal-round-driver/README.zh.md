...
驱动器在创建或明确恢复 goal 时捕获已认证的调用者。每个自动 Round 都会通过收件箱、轮次、模型调用归属和计费保留该 principal。工具创建的 goal 使用当前轮次的归属账号；Host Remote 调用使用 Gateway 已验证的调用者。编辑和无关提示不会转移这份授权。匿名本地 goal 仍然匿名，已结束的轮次不会隐式提供账号。
