...
describe('principal grouping', () => {
  it('claims authenticated and anonymous groups separately and preserves them on replay', async () => {
    const { ctx, session, agent, inbox } = await inboxAgent('principal-inbox')
    ...
    // Neutral internal context accompanies whichever caller the boundary selects,
    // and the FIFO prefix stops at the first message owned by someone else.
    expect(inbox.claim('next-step', 1)).toEqual([internal, a])
    expect(inbox.nextStep).toEqual([anonymous, b])

    const restored = new ReactLoopInbox(ctx.sessionProjections, session, agentEvents(ctx, agent))
