...
describe('tool-pipeline invariants', () => {
  ...
  it('distinguishes repeated root call ids and subcall ids by root call seq', async () => {
    const ctx = await setup()
    const session = ctx.sessions.create()
    session.append('turn/start', { turn: 1 })
    const firstRoot = session.append('tool/call', {
      turn: 1, step: 1, callId: ToolCallId('root'), name: 'run_code', arguments: '{}',
    })
    const first = {
      rootCallId: ToolCallId('root'),
      rootCallSeq: firstRoot.seq,
      parentCallId: ToolCallId('root'),
      subCallId: ToolCallId('root:code:1'),
      name: 'echo',
      arguments: {},
    }
    session.append('tool/code-dispatch-start', first)
    session.append('tool/code-dispatch', { ...first, isError: false, content: [] })

    const secondRoot = session.append('tool/call', {
      turn: 1, step: 2, callId: ToolCallId('root'), name: 'run_code', arguments: '{}',
    })
    const second = { ...first, rootCallSeq: secondRoot.seq }
    expect(() => session.append('tool/code-dispatch-start', second)).not.toThrow()
    expect(() => session.append('tool/code-dispatch', { ...second, isError: false, content: [] })).not.toThrow()
  })

  it('rejects settle and nested edges crossing repeated root-call lifecycles', async () => {
    const ctx = await setup()
    const session = ctx.sessions.create()
    session.append('turn/start', { turn: 1 })
    const firstRoot = session.append('tool/call', {
    ...
    session.append('tool/code-dispatch-start', parent)

    expect(() => session.append('tool/code-dispatch', {
      ...parent,
      rootCallSeq: secondRoot.seq,
      isError: false,
      content: [],
    })).toThrow(/no matching start in root-call lifecycle/)
    expect(() => session.append('tool/code-dispatch-start', {
      ...parent,
      rootCallSeq: secondRoot.seq,
      parentCallId: parent.subCallId,
      subCallId: ToolCallId('root:code:1:code:1'),
      name: 'echo',
    })).toThrow(/does not belong to rootCallId root at active rootCallSeq/)
