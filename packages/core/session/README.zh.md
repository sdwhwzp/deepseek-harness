...
- `sourceEventSeqs?: number[]`：被引用为来源的较早事件 seq（例如 `assistant/message` 引用的 `assistant/chunk` seq、`tool/result` 引用的准确 `tool/call`，或压缩替换条目引用的已遮蔽条目）。当提供方复用同一个不透明 call id 时，工具调用引用可区分不同调用。对于 `assistant/message`，存在的 `[]` 表示已知提供方流为空；省略则表示旧版或外部事件没有记录源流。其他 surface 事件若有此字段，则要求非空列表。
