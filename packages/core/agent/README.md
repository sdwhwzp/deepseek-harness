...
`PreStepDecision` is either `{ kind: 'reject' }` or `{ kind: 'enter', messages, startsRequestSeries? }`. The enter branch contains the complete identified, frozen message batch. `startsRequestSeries: true` declares a distinct model-message series; a wrapping listener preserves that declaration and the batch unless it intentionally replaces either one. Inbox claims preserve FIFO order within one caller group: authenticated groups compare `(source, id)`, while a principal-less `user` source is the anonymous group. Principal-less non-user messages are neutral internal context and accompany the selected caller. Claiming removes only the compatible prefix; input from another caller remains pending for a later turn.
