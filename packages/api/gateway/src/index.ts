...
export class TypertGatewayService extends Service implements TypertGateway {
        ...
        webCtx.effect(function* () {
          ...
          const route: WebUpgradeRoute = {
            ...
            handler: async (req, socket, head) => {
              const admission = await webCtx.connection.admitRequest(req)
  ...
  async invoke(request: InvokeRemoteRequest): Promise<unknown> {
    return this.withRequestPrincipal(this.principalForRequest(request), async () =>
  ...
  private async openStream(request: InvokeRemoteRequest, control: AbortController): Promise<AsyncIterable<unknown>> {
    ...
    const source = await this.withRequestPrincipal(principal, () => this.openPrincipalStream(request, control))
    return this.bindRequestPrincipal(source, principal)
  }

  private async openPrincipalStream(request: InvokeRemoteRequest, control: AbortController): Promise<AsyncIterable<unknown>> {
  ...
  private principalForRequest(request: Pick<InvokeRemoteRequest, 'principal' | 'peer'>): AuthenticatedPrincipal | undefined {
    return request.principal ?? (request.peer === undefined
      ? undefined
      : this.ctx.get('connection')?.principalOfPeer(request.peer))
  }
  ...
  private async dispatchRpc(
    ...
    const principal = this.principalForRequest({ peer })
  ...
  private async openWireStream(
    ...
    if (endpoint === REMOTE_EVENT_STREAM_ENDPOINT) {
      ...
      return this.openRemoteEvents(payload, signal, this.principalForRequest({ ...(peer === undefined ? {} : { peer }) }))
    }
  ...
  private async invokeRpc(
    ...
    return this.withRequestPrincipal(this.principalForRequest({ peer }), () =>
      this.invokePrincipalRpc(endpoint, payload, signal, peer))
  }

  private async invokePrincipalRpc(
    endpoint: string,
    payload: unknown,
    signal: AbortSignal,
    peer: PeerScope,
