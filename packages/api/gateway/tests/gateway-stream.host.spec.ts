...
import {
  ...
  type ConnectionPrincipalRequest,
  type RequestPrincipalProvider,
...
const ALICE_PRINCIPAL: AuthenticatedPrincipal = {
  source: 'gateway', id: 'alice', username: 'Alice', role: 'user',
}
const BOB_PRINCIPAL: AuthenticatedPrincipal = {
  source: 'gateway', id: 'bob', username: 'Bob', role: 'user',
}
type FixturePrincipalName = 'alice' | 'bob'
type AgentWireId = TypertContextWire<TypertContextMap['agent']>
const agentId = (value: string): AgentWireId => value as AgentWireId

const fixturePrincipalProvider: RequestPrincipalProvider = {
  async authenticate(request) {
    await Promise.resolve()
    const name = fixturePrincipalName(request)
    if (name === 'alice') return ALICE_PRINCIPAL
    if (name === 'bob') return BOB_PRINCIPAL
    if (name === 'reject') throw new Error('fixture principal rejected')
    return undefined
  },
}

function fixturePrincipalName(request: ConnectionPrincipalRequest): string | undefined {
  if (request.headers instanceof Headers) {
    return request.headers.get('x-fixture-principal') ?? undefined
  }
  const value = request.headers['x-fixture-principal']
...
class FeedService extends Service {
  ...
  readonly principalTrace: string[] = []
  returns = 0
  ...
  *src(label: string): Iterable<string> {
  ...
  @Remote({ mode: 'stream' })
  principalScope(label: string): AsyncIterable<string> {
    const observe = (operation: string): void => {
      this.principalTrace.push(
        `${operation}:${this.ctx.typertGateway.currentPrincipal()?.id ?? 'anonymous'}`,
      )
    }
    observe('method')
    let sent = false
      ...
      [Symbol.asyncIterator]: () => {
        ...
        return {
          next: () => {
            ...
            sent = true
            return Promise.resolve({ done: false, value: label })
          },
          return: () => {
            observe('return')
...
describe('Typert Remote streams', () => {
  ...
  it('restores the stream principal for method and iterator operations', async () => {
    const { ctx, service } = await setup(false)
    const alice: AuthenticatedPrincipal = {
      source: 'gateway', id: 'alice', username: 'Alice', role: 'user',
    }
    const returned = await ctx.typertGateway.stream({
      namespace: 'feed', method: 'principalScope', args: { label: 'named' }, principal: alice,
    })
    expect(ctx.typertGateway.currentPrincipal()).toBeUndefined()
    const iterator = returned[Symbol.asyncIterator]()
    await expect(iterator.next()).resolves.toEqual({ done: false, value: 'named' })
    await expect(iterator.return?.()).resolves.toEqual({ done: true, value: undefined })

    const thrown = await ctx.typertGateway.stream({
      namespace: 'feed', method: 'principalScope', args: { label: 'throw' }, principal: alice,
    })
    await expect(thrown[Symbol.asyncIterator]().throw?.(new Error('consumer stopped')))
      .resolves.toEqual({ done: true, value: undefined })

    const anonymous = await ctx.typertGateway.stream({
      namespace: 'feed', method: 'principalScope', args: { label: 'anonymous' },
    })
    await expect(anonymous[Symbol.asyncIterator]().next())
      .resolves.toEqual({ done: false, value: 'anonymous' })
    expect(service.principalTrace).toEqual([
      'method:alice', 'factory:alice', 'next:alice', 'return:alice',
      'method:alice', 'factory:alice', 'throw:alice',
      'method:anonymous', 'factory:anonymous', 'next:anonymous',
    ])
    expect(ctx.typertGateway.currentPrincipal()).toBeUndefined()
  })
  ...
  it('authenticates an upgrade before handoff and binds its principal to every stream', async () => {
    const { ctx, service } = await setup(true, {}, fixturePrincipalProvider)
    const rejectedSocket = new WebSocket(
      `ws://127.0.0.1:${String(ctx.webServer.port)}/api/remote.mux`,
      {
        headers: {
          cookie: browserCookie(ctx),
          'x-fixture-principal': 'reject',
        },
      },
    )
    rejectedSocket.on('error', () => {})
    const responseEvent: unknown[] = await once(rejectedSocket, 'unexpected-response')
    ...
    socket.on('message', (data) => {
      frames.push(JSON.parse(rawText(data)) as Record<string, unknown>)
    })
    sendOpen(socket, 'principal-a', 'feed/principalScope', { label: 'a' })
    sendOpen(socket, 'principal-b', 'feed/principalScope', { label: 'b' })
    await vi.waitFor(() => {
      expect(frames).toEqual(expect.arrayContaining([
        { type: 'item', streamId: 'principal-a', value: 'a' },
        { type: 'end', streamId: 'principal-a' },
        { type: 'item', streamId: 'principal-b', value: 'b' },
        { type: 'end', streamId: 'principal-b' },
      ]))
    })
    expect(service.principalTrace).toHaveLength(8)
    expect(service.principalTrace.every(entry => entry.endsWith(':alice'))).toBe(true)
    expect(ctx.typertGateway.currentPrincipal()).toBeUndefined()
  ...
  it('isolates Remote event delivery, replay, and results by exact principal', async () => {
    const { ctx } = await setup(true, {}, fixturePrincipalProvider)
    const source = new RemoteEventSourceProbe()
    const unregister = ctx.typertGateway.registerRemoteEvents(source.source, REMOTE_HOST)
    const agent = ctx.extend()
    ctx.typert.contexts.registerHost('agent', {
      wire: 'agentId',
      wireTypeSymbol: '@fixture#AgentId',
      identity: candidate => candidate === agent ? agentId('agent-principal') : undefined,
      resolve: id => id === 'agent-principal' ? agent : undefined,
    })
    ...
    const owned = pendingInvocation(agent, undefined, 'alice-owned', ALICE_PRINCIPAL)
    source.push(owned.dispatch)
    await vi.waitFor(() => {
      expect(deliveredInvocation(alice, 'alice-owned')).toBeDefined()
    })
    await new Promise<void>((resolve) => { setTimeout(resolve, 25) })
    expect(deliveredInvocation(bob, 'alice-owned')).toBeUndefined()
    expect(deliveredInvocation(anonymous, 'alice-owned')).toBeUndefined()
    const originalFrame = deliveredInvocation(alice, 'alice-owned')!
    expect(originalFrame).not.toHaveProperty('principal')

    source.push({
      event: 'fixture/principal-notification',
      args: ['alice-only'],
      principal: ALICE_PRINCIPAL,
    })
    await vi.waitFor(() => {
      expect(deliveredEmit(alice, 'fixture/principal-notification')).toEqual({
        type: 'emit', event: 'fixture/principal-notification', args: ['alice-only'],
      })
    })
    await new Promise<void>((resolve) => { setTimeout(resolve, 25) })
    expect(deliveredEmit(bob, 'fixture/principal-notification')).toBeUndefined()
    expect(deliveredEmit(anonymous, 'fixture/principal-notification')).toBeUndefined()

    const closed = once(alice.socket, 'close')
    alice.socket.close()
    await closed
    const replacement = await openEventClient(ctx, 'events-alice-replacement', 'alice')
    await vi.waitFor(() => {
      expect(deliveredInvocation(replacement, 'alice-owned')).toBeDefined()
    })
    const replayed = deliveredInvocation(replacement, 'alice-owned')!
    expect(replayed.eventId).toBe(originalFrame.eventId)
    expect(deliveredInvocation(bob, 'alice-owned')).toBeUndefined()
    expect(deliveredInvocation(anonymous, 'alice-owned')).toBeUndefined()

    await expect(sendEventResult(bob, replayed, { kind: 'result', value: 'stolen' }))
      .rejects.toThrow('principal does not own its Client and pending event')
    await expect(sendEventResult(
      { ...replacement, principalName: 'bob' },
      replayed,
      { kind: 'result', value: 'stolen' },
    )).rejects.toThrow('principal does not own its Client and pending event')
    expect(owned.resolve).not.toHaveBeenCalled()
    expect(owned.reject).not.toHaveBeenCalled()

    await sendEventResult(replacement, replayed, { kind: 'result', value: 'allowed' })
    await expect(owned.outcome).resolves.toEqual({ kind: 'result', value: 'allowed' })

    const anonymousPending = pendingInvocation(agent, undefined, 'anonymous-owned')
    source.push(anonymousPending.dispatch)
    await vi.waitFor(() => {
      expect(deliveredInvocation(anonymous, 'anonymous-owned')).toBeDefined()
    })
    await new Promise<void>((resolve) => { setTimeout(resolve, 25) })
    expect(deliveredInvocation(replacement, 'anonymous-owned')).toBeUndefined()
    expect(deliveredInvocation(bob, 'anonymous-owned')).toBeUndefined()
    await sendEventResult(
      anonymous,
      deliveredInvocation(anonymous, 'anonymous-owned')!,
      { kind: 'next' },
    )
    await expect(anonymousPending.outcome).resolves.toEqual({ kind: 'next' })
  ...
  it('filters Session-addressed notifications through each Client principal access grant', async () => {
    const { ctx } = await setup(true, {}, fixturePrincipalProvider)
    ctx.provide('principalAccess', {
      resolve: async (
        principal: AuthenticatedPrincipal,
        subjects: { readonly sessionIds?: readonly ReturnType<typeof SessionId>[] },
      ) => ({
        readableSessionIds: new Set((subjects.sessionIds ?? [])
          .filter(sessionId => String(sessionId) === `session-${principal.id}`)),
        readableWorkspaceIds: new Set(),
      }),
    } as never)
    const source = new RemoteEventSourceProbe()
    const unregister = ctx.typertGateway.registerRemoteEvents(source.source, REMOTE_HOST)
    const alice = await openEventClient(ctx, 'events-access-alice', 'alice')
    const bob = await openEventClient(ctx, 'events-access-bob', 'bob')
    const anonymous = await openEventClient(ctx, 'events-access-anonymous')
    const sessionId = SessionId('session-alice')
    ...
    await vi.waitFor(() => {
      expect(deliveredEmit(alice, 'api-session/status')).toEqual({
        type: 'emit', event: 'api-session/status', args: [sessionId, true],
      })
    })
    await new Promise<void>((resolve) => { setTimeout(resolve, 25) })
    expect(deliveredEmit(bob, 'api-session/status')).toBeUndefined()
    expect(deliveredEmit(anonymous, 'api-session/status')).toBeUndefined()

    source.push({ event: 'settings/document-updated', args: ['theme', 1] })
    await vi.waitFor(() => {
      expect(deliveredEmit(alice, 'settings/document-updated')).toBeDefined()
      expect(deliveredEmit(bob, 'settings/document-updated')).toBeDefined()
      expect(deliveredEmit(anonymous, 'settings/document-updated')).toBeDefined()
    })
...
async function setup(
  ...
  principalProvider?: RequestPrincipalProvider,
): Promise<{ readonly ctx: Context; readonly service: FeedService }> {
  const ctx = new Context()
  roots.push(ctx)
  if (principalProvider !== undefined) ctx.provide('requestPrincipal', principalProvider)
  if (transport) {
...
function descriptors(): InvocationDescriptor[] {
  ...
  return [
    ...
    stream('principalScope', [label], z.string()),
    {
...
interface RemoteEventTestClient {
  ...
  readonly principalName?: FixturePrincipalName
}

async function openEventClient(
  ctx: Context,
  streamId: string,
  principalName?: FixturePrincipalName,
  ...
  const socket = new WebSocket(`${origin.replace('http:', 'ws:')}/api/remote.mux`, {
    headers: {
      cookie,
      ...principalName === undefined ? {} : { 'x-fixture-principal': principalName },
    },
  })
  await once(socket, 'open')
  const frames: Record<string, unknown>[] = []
...
function deliveredInvocation(
  client: RemoteEventTestClient,
  prompt?: string,
): RemoteEventInvocationFrame | undefined {
  for (const frame of client.frames) {
    ...
    const invocation = value as RemoteEventInvocationFrame
    if (prompt !== undefined && invocation.request.prompt !== prompt) continue
    return invocation
  }
  return undefined
}

function deliveredEmit(
  client: RemoteEventTestClient,
  event: string,
  ...
  for (const frame of client.frames) {
    ...
    if (Reflect.get(value, 'type') !== 'emit' || Reflect.get(value, 'event') !== event) continue
...
async function sendEventResult(
  ...
  const response = await fetch(`${client.origin}/api/$events/result`, {
    ...
    headers: {
      ...
      ...client.principalName === undefined
        ? {}
        : { 'x-fixture-principal': client.principalName },
    },
    body: JSON.stringify({
      type: 'client-request',
      rpcId,
