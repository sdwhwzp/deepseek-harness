...
The Host entry registers one interceptor on Connection's shared `/api` FetchHandler. Connection supports multiple disjoint interceptors and fails an ambiguously claimed endpoint instead of selecting by registration order. It passes the composite handler through its HTTP bridge; the handler dispatches claimed endpoints to Gateway and returns 404 for unclaimed requests unless an exact Fetch route owns them. Direct `invoke()` calls preserve business errors; `TypertGatewayError` is a `RemoteError` subclass whose `gateway/*` codes name the failures owned by dispatch, binding, providers, lookup, Context, arguments, and codecs. A resolver that refuses on policy grounds — a cold-resume failure or an ownership fence — throws its own `RemoteError`, and the code it chose reaches the caller unchanged.

Connection passes its transport-verified `AuthenticatedPrincipal` as a Host-only invocation field. Gateway scopes the value through `AsyncLocalStorage` for one unary call or for each advancement of one stream iterator; a composed business service reads it through `ctx.typertGateway.currentPrincipal()`, which returns undefined outside dispatch. Browser arguments cannot populate or retain this context, and concurrent calls and streams do not share it.
