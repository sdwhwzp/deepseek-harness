...
Connection passes its transport-verified `AuthenticatedPrincipal` as a host-only invocation field. Every public `invoke()` and `stream()` call installs an explicit `AsyncLocalStorage` scope, including anonymous calls; nested calls replace that scope and restore their caller afterward. A composed business service reads the active identity through `ctx.typertGateway.currentPrincipal()`. Stream method invocation and every iterator `next()`, `return()`, and `throw()` run in the bound scope. Named browser arguments cannot populate or retain this context, and concurrent calls do not share it.
...
Gateway binds each `$events` Client generation to its handshake principal. A pending Agent-scoped waterfall retains its initiating principal and is delivered or replayed only to Clients with the same `(source, id)`; anonymous pending work reaches only anonymous Clients. `$events/result` requires the HTTP request, Client generation, and pending event principals to match before settlement state changes.

An ordinary event attributed to the active request is filtered by that exact principal. A source may also attach process-local `readSubjects`; Gateway resolves `ctx.principalAccess` separately for every Client and publishes the event only when that Client can read every named Session or Workspace. Only notifications with neither principal attribution nor read subjects are global broadcasts. Principal and read-subject fields never enter Remote event wire frames.
...
- Forwarded event payloads reach `$on` after event-level principal and resource checks, but Gateway performs no field-level business-payload projection or redaction. Ordinary notifications are not replayed after reconnect; Agent-scoped waterfalls project only the top-level Agent identity needed to select the Client Context and carry their own pending lifetime.
