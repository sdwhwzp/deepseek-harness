...
Connection 把其传输层已验证的 `AuthenticatedPrincipal` 作为仅 Host 可写的调用字段传入。每次公开 `invoke()` 和 `stream()` 调用都会建立显式 `AsyncLocalStorage` scope，匿名调用也不例外；嵌套调用会替换该 scope，并在结束后恢复调用方的 scope。已组合的业务服务通过 `ctx.typertGateway.currentPrincipal()` 读取活动身份。stream method 调用以及 iterator 的每次 `next()`、`return()` 和 `throw()` 都在绑定的 scope 中运行。浏览器具名参数不能填充或保留该上下文，并发调用也不会共享它。
...
流式 Remote 使用 `@Remote({ mode: 'stream' })` 并返回 `Iterable` 或 `AsyncIterable`。`ctx.typertGateway.stream()` 执行与一元调用相同的 endpoint、参数、lookup 和取消校验，再用生成的 result codec 校验每个产出项。Client 插件激活时打开 Gateway 自有的 `/api/remote.mux` WebSocket，使其在空闲时保持连接，并以有上限的退避重试物理连接失败。Host 会在 handoff 前异步认证 upgrade，认证失败返回 401，并把已验证 principal 绑定到物理 socket 及其每条逻辑流。Host 按配置的 `websocketHeartbeatIntervalMs` 间隔（默认 30 秒）发送 Ping 控制帧，浏览器在 WebSocket 协议层自动回复 Pong，使空闲网络中间层持续看到流量，而不新增 Remote stream frame。可独立取消的逻辑流共享这条连接；进程内 Connection 载体直接提供等价的流，不打开该 WebSocket。
...
Gateway 把每个 `$events` Client generation 绑定到其握手 principal。待处理的 Agent-scoped waterfall 会保留发起 principal，并且只向 `(source, id)` 相同的 Client 投递或重放；匿名待处理工作只会到达匿名 Client。`$events/result` 会在修改结算状态前要求 HTTP 请求、Client generation 与待处理事件的 principal 全部匹配。

能够归因到活动请求的普通事件会按该精确 principal 过滤。事件 source 还可以附带只存在于进程内的 `readSubjects`；Gateway 为每个 Client 分别解析 `ctx.principalAccess`，仅在该 Client 可读取全部具名 Session 或 Workspace 时才发布事件。只有既无 principal 归因也无 read subject 的通知才会全局广播。Principal 与 read-subject 字段都不会进入 Remote 事件 wire frame。
...
- 转发事件的载荷会在事件级 principal 与资源检查之后到达 `$on`，但 Gateway 不执行字段级业务载荷投影或脱敏。普通通知在重连后不重放；Agent-scoped waterfall 只投影选择 Client Context 所需的顶层 Agent 身份，并自行携带 pending 生命周期。
