...
- 被转发的事件到达 `$on` 时不做业务载荷投影或脱敏。普通通知在重连后不重放；API Remotes 把请求发起的通知归属到当前 principal，并给以 Session 寻址的通知附上进程内读取主体，Gateway 再为每条物理 Client 世代独立过滤后入队；Agent-scoped waterfall 在挂起与重放期间保留发起方的 principal。Agent-scoped waterfall 只投影选择 Client Context 所需的顶层 Agent 身份，并自行携带 pending 生命周期。
