...
The Host entry registers one allowlist listener set and source queue, which Gateway fans into per-Client queues. It rejects non-JSON ordinary-event arguments before enqueueing and captures the current request principal when one exists. Session Controller notifications additionally carry process-local `readSubjects` derived from their declared Session argument, so Gateway resolves `ctx.principalAccess` for every Client before delivery. Notifications with neither attribution nor read subjects are the only global broadcasts, and neither process-local field enters the wire event frame. For a waterfall, the source projects only the top-level Agent identity and JSON request fields; a Client result must also be lossless JSON, while `next()` delegates to the following Host listener. The source attaches all listeners synchronously before `ctx.typertGateway.registerRemoteEvents()` exposes Gateway's internal `$events` logical stream, so its first `ready` item proves that incremental delivery is active and carries the Host home for Client path display. Withdrawing the registration aborts active streams.
