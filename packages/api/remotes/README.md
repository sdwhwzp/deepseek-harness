...
The Host entry registers an independent allowlist listener set and queue for each Client stream. It rejects non-JSON ordinary-event arguments before enqueueing. Request-originated events capture the active Gateway principal, Session-addressed notifications carry process-local read subjects, and a waterfall recovers its owner from the active step or turn when dispatch runs after the initiating request. Gateway applies these fields separately to every Client generation; neither field enters the wire frame. For a waterfall, the source projects only the top-level Agent identity and JSON request fields; a Client result must also be lossless JSON, while `next()` delegates to the following Host listener. The source attaches all listeners synchronously before `ctx.typertGateway.registerRemoteEvents()` exposes Gateway's internal `$events` logical stream, so its first `ready` item proves that incremental delivery is active and carries the Host home for Client path display. Withdrawing the registration aborts active streams.
