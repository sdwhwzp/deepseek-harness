...
Host entry 注册一套 allowlist listener 与一个源队列，再由 Gateway 将该队列分发到每个 Client 的队列。普通事件入队前会拒绝非 JSON 参数，并在当前请求存在 principal 时捕获该身份。Session Controller 通知还会根据其声明的 Session 参数携带只存在于进程内的 `readSubjects`，使 Gateway 在投递前为每个 Client 解析 `ctx.principalAccess`。只有既无身份归因也无 read subject 的通知才会全局广播，这两个进程内字段都不会进入 wire 事件 frame。对于 waterfall，source 只投影顶层 Agent 身份与 JSON 请求字段；Client 结果也必须能无损表示为 JSON，而 `next()` 会委托给后续 Host listener。该 source 在 `ctx.typertGateway.registerRemoteEvents()` 暴露 Gateway 内部的 `$events` logical stream 前同步挂好所有 listener，因此首个 `ready` 项既能证明增量投递已就绪，也会携带供 Client 显示路径的 Host home。撤回注册会中止活动 stream。
