      ...
      events: [
        {
          ...
          data: { turn: 1, ...principal === undefined ? {} : { principal } },
        },
        {
          type: 'step/start', seq: 1, time: 2,
          data: { turn: 1, step: 1, ...principal === undefined ? {} : { principal } },
        },
      ],
    },
  } as unknown as Agent
...
describe('Remote event Host source', () => {
  ...
  it('carries a safely attributable ordinary-event principal only in process', async () => {
    const { ctx, gateway } = await setup()
  ...
  it('marks Session notifications for per-client read authorization', async () => {
    const { ctx, gateway } = await setup()
    const abort = new AbortController()
    const iterator = sourceOf(gateway)(abort.signal)[Symbol.asyncIterator]()
    ...
    emitRaw(ctx, 'api-session/added', [{ sessionId, updatedAt: 1 }])
    await expect(iterator.next()).resolves.toMatchObject({
      done: false,
      value: { readSubjects: { sessionIds: [sessionId] } },
    })
  ...
  it('bridges scoped waterfall result, next delegation, and rejection', async () => {
    ...
    const agent = fakeAgent(agentCtx, alice)
  ...
  it('uses the active step owner, then the open turn owner, for waterfalls', async () => {
    const { ctx, gateway } = await setup()
    const abort = new AbortController()
    const iterator = sourceOf(gateway)(abort.signal)[Symbol.asyncIterator]()
    const alice: AuthenticatedPrincipal = {
    ...
    const agent = {
      id: 'fixture-agent', ctx: agentCtx, session: { events },
    } as unknown as Agent
    const target = scopeTarget(ctx, agent)
    const request = { questions: [], agent }
    ...
    events.push({ type: 'turn/end', seq: 3, time: 4, data: { turn: 1, reason: 'complete' } })
    const closed = waterfallRaw(
      ctx, target, 'user-questions/request', [request], () => Promise.resolve('next'),
    )
    const closedDispatch = invocationOf((await iterator.next()).value)
    expect(closedDispatch.principal).toBeUndefined()
    closedDispatch.resolve({ kind: 'next' })
    await expect(closed).resolves.toBe('next')
