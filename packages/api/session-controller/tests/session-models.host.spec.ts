...
describe('Web session model selection', () => {
  ...
  it.each([
    { configuredKey: false, authenticated: false },
    { configuredKey: true, authenticated: false },
    { configuredKey: false, authenticated: true },
    { configuredKey: true, authenticated: true },
  ])('account login initializes a default with API key=$configuredKey, administrator=$authenticated', async ({ configuredKey, authenticated }) => {
    ...
    const principal: AuthenticatedPrincipal = { source: 'fixture', id: 'admin', username: 'admin', role: 'admin' }
    const resolve = vi.fn(async () => ({ readableSessionIds: new Set(), readableWorkspaceIds: new Set() }))
    if (authenticated) {
    ...
    if (authenticated) expect(resolve).toHaveBeenCalledWith(principal, {}, undefined)
