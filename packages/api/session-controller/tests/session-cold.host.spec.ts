...
describe('subagent ownership fence', () => {
  ...
  it('does not classify an ordinary fork from an inherited ancestor descriptor and preserves its principal', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(AgentRegistry)
    ...
    expect(response.ok).toBe(true)
    expect(followup).toHaveBeenCalledWith(expect.objectContaining({ principal }))
  })
