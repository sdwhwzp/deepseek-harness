...
describe('session/openWorkspacePath', () => {
  ...
  it('preserves relative and absolute Host-resolvable paths', async () => {
    ...
    await remote.openWorkspacePath({ sessionId: SessionId('open-path'), path: '/tmp/result.html' })
    await remote.openWorkspacePath({ sessionId: SessionId('open-path'), path: 'result.html' })
