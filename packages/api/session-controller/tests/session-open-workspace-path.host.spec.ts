...
describe('session/openWorkspacePath', () => {
  ...
  it('normalizes relative and absolute Host-resolvable paths', async () => {
    ...
    await remote.openWorkspacePath({ sessionId, path: '/workspace/result.html' })
...
it('reports Host file-manager metadata and dispatches reveal separately from default-app open', async () => {
  ...
  try {
    ...
    const signal = new AbortController().signal
    await controller.openWorkspacePath({ sessionId, path: '/workspace/report.txt', action: 'reveal' }, signal)
...
it('uses the native reveal adapter without a test override and respects unsupported desktop metadata', async () => {
  ...
  try {
    ...
    await controller.openWorkspacePath({ sessionId, path: '/report.txt', action: 'reveal' }, new AbortController().signal)
...
it.each(['open', 'reveal'] as const)('rejects an unmapped remote path before native %s', async (action) => {
  ...
  await expect(controller.openWorkspacePath({ sessionId, path: '/remote/report.html', ...(action === 'reveal' ? { action } : {}) }, new AbortController().signal))
...
it('rejects a filesystem mapping that resolves to another process path', async () => {
  ...
  await expect(controller.openWorkspacePath({ sessionId, path: '/report.html' }, new AbortController().signal))
    .rejects.toMatchObject({ code: 'gateway/bad-request', message: 'Path has no verified Host path' })
  expect(openPath).not.toHaveBeenCalled()
})
...
it('dispatches default-app opening to the association adapter', async () => {
  ...
  const signal = new AbortController().signal
  await controller.openWorkspacePath({ sessionId, path: '/report.html' }, signal)
...
it('uses the same path verification for handler queries and explicit application opening', async () => {
  ...
  const signal = new AbortController().signal
  expect(await controller.workspacePathApplications({ sessionId, path: '/file.mp3' }, signal)).toEqual(applications)
  await controller.openWorkspacePath({ sessionId, path: '/file.mp3', application: applications[0]!.id }, signal)
  expect(openFileApplication).toHaveBeenCalledWith(expect.stringContaining('file.mp3'), applications[0]!.id, signal)
  const mapping = vi.spyOn(ctx.fs, 'processPathFromHostPath').mockReturnValue(undefined)
  onTestFinished(() => { mapping.mockRestore() })
  await expect(controller.workspacePathApplications({ sessionId, path: '/remote.mp3' }, signal)).rejects.toThrow('verified Host path')
  await expect(controller.openWorkspacePath({ sessionId, path: '/remote.mp3', application: applications[0]!.id }, signal)).rejects.toThrow('verified Host path')
...
it('avoids desktop queries when unavailable and rejects an empty query path', async () => {
  ...
  expect(await unavailable.workspacePathApplications({ sessionId, path: '/file.mp3' }, new AbortController().signal)).toEqual([])
  ...
  await expect(available.workspacePathApplications({ sessionId, path: '' }, new AbortController().signal)).rejects.toMatchObject({ code: 'gateway/bad-request' })
...
it('returns bounded query failures and classifies cancellation through the Remote', async () => {
  ...
  await expect(remote.workspacePathApplications({ sessionId, path: '/file.mp3' })).resolves.toMatchObject({
    ok: false, error: { code: 'gateway/internal', message: 'file application query failed' },
  })
  await expect(remote.workspacePathApplications({ sessionId, path: '/file.mp3' }, lifetime.signal)).resolves.toMatchObject({
