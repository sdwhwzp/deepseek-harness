...
describe('SessionController subagent catalog', () => {
  ...
  it('reads registered projections without a subagent provider or catalog key', async () => {
    ...
    await expect(controller.projections({ sessionId: PARENT }, new AbortController().signal)).resolves.toEqual({
      ...expected, values: { ...expected.values, imageLimits: TEST_IMAGE_LIMITS },
    })
