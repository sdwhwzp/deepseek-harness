...
凡是按现有 Session 寻址的操作，都会捕获 Gateway 中传输层已验证的 principal，并在观察、恢复或变更之前授权该精确 Session。列表与搜索在 summary 投影、空白产物探测或内容排序之前授权候选 id。follow 与 control stream 会授权 opening 数据，并对每个按资源寻址的 frame 重新检查，因此撤销访问后不会发布下一帧。direct-subagent 地址先授权其普通父 Session，再应用独立的 lineage 校验。只有在请求认证与 principal-access 提供方都未挂载时，本地匿名组合才保留既有访问；不完整的认证组合会严格拒绝。
