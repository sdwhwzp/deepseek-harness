...
`session.prompt` 从当前 API Gateway 调用读取经传输层验证的 principal，并将其存入新的持久 `UserMessage`。匿名直接调用不带该字段；请求参数无法设置它。

每个指向既有 Session 的 Remote 操作都会先通过 `ctx.principalAccess` 解析同一个消息作用域 principal，再读取、恢复、变更该 Session 或为其打开路径。列表在构建摘要或探测冷工件前授权候选 id，搜索把匹配项限制为已授权候选 id，page 在加载日志前拒绝不可读的普通 Session 或 subagent 父 Session。follow 在加载前以及发布每个 frame 前都重新检查，因此撤权会终止 stream，且不会再发布 frame；control 同时过滤 opening baseline 与每个实时 frame。skill 与文件引用目录使用相同检查。因此 `session.openWorkspacePath` 除了解析后的 Host 路径，还要求提供所属 `sessionId`。create、模型目录与原生打开器能力探测不指向既有 Session。

principal-access 提供方、请求认证提供方与 principal 都不存在时保留本地匿名旧有访问。认证组合缺少 principal-access 提供方或当前 principal 时都会关闭式失败；两者都存在时，部署提供方针对其账户数据验证 `(source, id)` 并返回可读 Session id。此资源可见性前置条件不取代部署特定的变更策略。仅凭 `username` 与 `role` 绝不授予访问权。
...
- 文件引用补全可在访问检查通过后恢复已授权的冷 Session；`skills/list` 目录是不激活 Agent 的 skill 元数据读取路径。
