...
import { errorChain, type AuthenticatedPrincipal } from '@deepseek-ai/dsh-llm'
...
export class SessionController extends TypertRemoteService {
  ...
  /**
   * Read authorized Session rows without resuming an Agent or probing denied artifacts.
  ...
  /**
   * Select one authorized Session-local model after explicitly resuming the Session.
  ...
  /**
   * Open one path prepared by an authorized Session-aware caller on the Host desktop.
   * @param request - owning Session and path after best-effort workspace resolution.
  ...
  /**
   * Rename one authorized Session after explicitly resuming it.
  ...
  /**
   * Fork one authorized cold-readable completed-turn prefix into a new Session.
  ...
  /**
   * Admit one prompt after authorizing and explicitly resuming its Session.
  ...
  /**
   * Read one image proven reachable from an authorized Session log.
  ...
  /**
   * Mutate one still-pending queue occurrence on an authorized live Agent.
  ...
  /**
   * Cancel one authorized active Agent turn without dropping its pending inbox.
   ...
   * @returns a complete opening snapshot followed by gap-free frames while access remains granted.
   */
  @Remote({ mode: 'stream' })
  ...
  private async resolveReadableSessionIds(
    ...
    return (await resolvePrincipalAccess(this.ctx, principal, { sessionIds }, signal))
      .readableSessionIds
  }
  ...
  private async requireReadableAddress(
    ...
    if (address.kind === 'session') {
      throw new RemoteError(
        'session/not-found',
        `session "${address.sessionId}" not found`,
