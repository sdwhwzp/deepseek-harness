...
Authenticated deployments leave first-use directory creation to their account-owned folder flow; `initializeDefault` returns `undefined` after validating the caller. Pin, unpin, and unarchive requests authorize the addressed Session and filter all returned identities.
