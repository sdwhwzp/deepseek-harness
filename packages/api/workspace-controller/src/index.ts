...
export class WorkspaceController extends TypertRemoteService {
  ...
  async initializeDefault(signal: AbortSignal): Promise<WorkspaceValue | undefined> {
    if (this.currentPrincipal() !== undefined || this.ctx.get('requestPrincipal') !== undefined || this.ctx.get('principalAccess') !== undefined) {
      await resolvePrincipalAccess(this.ctx, this.currentPrincipal(), {}, signal)
      // Deployment-owned account directories are registered through the authorized folder flow.
      return undefined
    }
  ...
  unarchiveSession(request: WorkspaceUnarchiveSessionRequest): Promise<WorkspaceArchiveValue> {
    return this.commands.unarchiveSession(request, this.currentPrincipal())
  }
  ...
  pinSession(request: WorkspacePinSessionRequest): Promise<WorkspacePinValue> {
    return this.commands.pinSession(request, this.currentPrincipal())
  }
  ...
  unpinSession(request: WorkspaceUnpinSessionRequest): Promise<WorkspacePinValue> {
    return this.commands.unpinSession(request, this.currentPrincipal())
  }
