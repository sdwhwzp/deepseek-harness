...
describe('principalWorkspaceFollow', () => {
  it('preserves the complete local projection when neither principal nor provider is configured', async () => {
    const ctx = new Context()
    const one = workspace('one', ['session-one'])
    await expect(collect(ctx, undefined,
      {
        type: 'baseline',
        value: { items: [one], archivedSessionIds: [SessionId('archived')] },
      },
  ...
  it('fails closed for either half of an authenticated deployment', async () => {
    const withoutProvider = collect(new Context(), PRINCIPAL, {
      type: 'baseline', value: { items: [], archivedSessionIds: [] },
    })
    await expect(withoutProvider).rejects.toMatchObject({
  ...
  it('filters the baseline and every increment without revealing undisclosed removals', async () => {
    const ctx = new Context()
