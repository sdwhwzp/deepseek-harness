...
describe('WorkspaceController commands', () => {
  it('authorizes pins and unarchive before mutation and declines shared-account first-use directories', async () => {
    const { controller, ctx } = await harness()
    const own = SessionId('alice-session')
    const hidden = SessionId('bob-session')
    ctx.sessions.create(own)
    ctx.sessions.create(hidden)
    await ctx.workspaceRegistry.pinSession(hidden)
    const principal: AuthenticatedPrincipal = { source: 'fixture', id: 'alice', username: 'Alice', role: 'user' }
    ctx.provide('typertGateway', { currentPrincipal: () => principal } as never)
    ctx.provide('principalAccess', {
      resolve: async () => ({ readableSessionIds: new Set([own]), readableWorkspaceIds: new Set() }),
    } as never)
    await expect(controller.pinSession({ sessionId: own })).resolves.toEqual({ pinnedSessionIds: [own] })
    for (const operation of [
      () => controller.pinSession({ sessionId: hidden }),
      () => controller.unpinSession({ sessionId: hidden }),
      () => controller.unarchiveSession({ sessionId: hidden }),
    ]) await expect(operation()).rejects.toMatchObject({ code: 'session/not-found' })
    expect(ctx.workspaceRegistry.pinnedSessionIds).toContain(hidden)
    await expect(controller.unpinSession({ sessionId: own })).resolves.toEqual({ pinnedSessionIds: [] })
    const initialize = vi.spyOn(ctx.workspaceRegistry, 'initializeDefault')
    await expect(controller.initializeDefault({ directoryName: 'shared', title: 'Shared' }, new AbortController().signal)).resolves.toBeUndefined()
