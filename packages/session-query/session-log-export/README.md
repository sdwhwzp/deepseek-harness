...
The Host authorizes the root Session and the complete requested descendant tree before it flushes or reads any raw artifact. A denied Session is reported as not found; an authenticated request without a principal-access provider reports service unavailable, and a provider-backed request without a principal reports unauthorized. Local anonymous deployments with no provider retain full export access.
...
The Host route is a feature-owned exact Fetch contribution. Connection applies its Host/Origin and browser-session checks, attaches the transport-verified principal, and bridges the streaming `Response`; this package resolves the complete lineage, applies `ctx.principalAccess`, then owns live-session flushes, raw artifact and attachment reads, ZIP generation, and HTTP status semantics. An all-or-nothing authorization pass finishes before the response body is created, so a rejected descendant cannot produce a partial archive.
...
- [dsh-principal-access](../../identity/principal-access/README.md) — the deployment-owned Session and Workspace read authorization provider.
