...
The Host route is a feature-owned exact Fetch contribution. Connection applies its Host/Origin and transport-principal or browser-session checks and bridges the streaming `Response`; this package owns query validation, live-session flushes, handle-based log and attachment reads, ZIP generation, and HTTP status semantics. The route authorizes the root Session before tracing lineage and, when descendants are requested, authorizes the complete descendant set before reading any log. Denied and absent Sessions share the not-found response, and no partial archive is produced.
