...
Host 路由是业务拥有的精确 Fetch contribution。Connection 应用 Host/Origin 以及传输层 principal 或浏览器会话检查，并桥接流式 `Response`；本包拥有查询校验、活动会话 flush、基于句柄的日志与附件读取、ZIP 生成和 HTTP 状态语义。该路由在追踪 lineage 之前授权根 Session；请求 descendants 时，会在读取任何日志之前授权完整 descendant 集合。被拒绝与不存在的 Session 使用同一 not-found 响应，并且不会生成部分归档。
