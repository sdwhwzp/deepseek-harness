...
Host 会在 flush 或读取任何原始产物之前，授权根 Session 和本次请求的完整后代树。被拒绝的 Session 按不存在报告；认证请求缺少 principal-access provider 时报告服务不可用，provider 支持的请求缺少 principal 时报告未认证。未配置 provider 的本地匿名部署保留完整导出访问。
...
Host 路由是业务拥有的精确 Fetch contribution。Connection 应用 Host/Origin 与浏览器会话检查、附加由传输验证的 principal，并桥接流式 `Response`；本包解析完整 lineage、应用 `ctx.principalAccess`，然后拥有活动会话 flush、原始产物与附件读取、ZIP 生成和 HTTP 状态语义。全有或全无的授权在响应 body 创建前完成，因此被拒绝的后代不会产生部分归档。
...
- [dsh-principal-access](../../identity/principal-access/README.zh.md)——部署拥有的 Session 与 Workspace 读取授权 provider。
