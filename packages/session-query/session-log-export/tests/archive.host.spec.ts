...
describe('session.export download endpoint', () => {
  ...
  it('forwards one request signal through root, lineage, and descendant reads', async () => {
    ...
    // Authorization traces the lineage on the request signal before the stream
    // exists; the descendant read then runs on the producer signal derived from
    // it, so one request cancellation still reaches all three reads.
    ...
    for (const signal of [rootSignal, lineageSignal, descendantSignal]) {
