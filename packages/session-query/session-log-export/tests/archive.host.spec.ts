...
async function buildApi(
  ...
  if (services.principal !== undefined) {
    ctx.provide('requestPrincipal', {
      authenticate: () => services.principal,
    } as never)
  }
  if (services.principalAccess !== undefined) {
    ctx.provide('principalAccess', { resolve: services.principalAccess } as never)
  }
...
describe('session.export download endpoint', () => {
  ...
  it('fails closed before lineage or artifact reads when principal configuration is incomplete', async () => {
    ...
    const readRaw = vi.fn(async () => artifact('session-root'))
    const principalOnly = await buildApi({}, [], { principal, traceSession, readRaw })
    const missingProvider = await principalOnly.fetch.fetch(
      new Request('http://host/api/session.export?sessionId=session-root'),
    )
    ...
    const resolve = vi.fn(async (): Promise<PrincipalAccessResult> => ({
      readableSessionIds: new Set(), readableWorkspaceIds: new Set(),
    }))
    const providerOnly = await buildApi({}, [], { principalAccess: resolve, traceSession, readRaw })
    const missingPrincipal = await providerOnly.fetch.fetch(
      new Request('http://host/api/session.export?sessionId=session-root'),
    )
  ...
  it('rejects a denied root or descendant before any raw artifact or attachment read', async () => {
    for (const method of ['GET', 'HEAD'] as const) {
      const readRaw = vi.fn(async () => artifact('session-root'))
      const readImage = vi.fn(async (ref: ImageAttachmentRef) => storedImage(String(ref.attachmentId)))
      const rootDenied = await buildApi({}, [], {
        principal,
        readRaw,
        attachments: readImage,
        principalAccess: async () => ({
          readableSessionIds: new Set(), readableWorkspaceIds: new Set(),
        }),
      })
      const rootResponse = await rootDenied.fetch.fetch(new Request(
        'http://host/api/session.export?sessionId=session-root',
        { method },
      ))
      ...
      const resolve = vi.fn(async (
        ...
        readableSessionIds: new Set(resolve.mock.calls.length === 1
          ? subjects.sessionIds
          : []),
        readableWorkspaceIds: new Set(),
      }))
      const descendantDenied = await buildApi({}, [node('child-a')], {
        principal,
        readRaw,
        attachments: readImage,
        principalAccess: resolve,
      })
      const descendantResponse = await descendantDenied.fetch.fetch(new Request(
        'http://host/api/session.export?sessionId=session-root&includeDescendants=true',
        { method },
      ))
      expect(descendantResponse.status).toBe(404)
  ...
  it('forwards one request signal through root, lineage, and descendant reads', async () => {
    ...
    const lineageSignal = traces[0]
    if (lineageSignal === undefined) throw new Error('missing lineage signal')
    ...
    expect(lineageSignal).toBe(rootSignal)
