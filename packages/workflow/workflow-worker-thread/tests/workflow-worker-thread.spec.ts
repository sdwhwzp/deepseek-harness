...
describe('dsh-workflow-worker-thread', { timeout: 120_000 }, () => {
  ...
  describe('lifecycle: parse errors, cancellation, termination, disposal', () => {
    ...
    it("carries the delegating step's owner to every child the script spawns", async () => {
      ...
      ctx.subagents.registerProvider(provider)
      await ctx.plugin(WorkerThreadWorkflowEngine, { provider: 'owner-probe', maxConcurrentAgents: 2 })
    ...
    it('a run started without an owner leaves its children unowned rather than guessing', async () => {
      ...
      await ctx.plugin(WorkerThreadWorkflowEngine, { provider: 'owner-absent', maxConcurrentAgents: 1 })
      const handle = ctx.workflowEngine.start({
        ...scripted(`
          await agent('one')
          return 'done'
