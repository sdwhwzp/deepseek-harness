...
A child start snapshots the parent tool execution's authenticated principal onto the child prompt. Continuable follow-ups remember the principal accepted with each message, and parent reports, settlement messages, browser prompts, control operations, and cold resume propagate that principal rather than consulting mutable Session or connection identity.
