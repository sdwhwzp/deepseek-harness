...
Continuable children answer follow-up messages as their next turns, and the parent can interrupt a running turn or list its children at any time. The browser `prompt` Remote snapshots the transport-verified Gateway principal on the accepted child message and on that activation's eventual settlement notice; a later accepted follow-up replaces the settlement owner. Anonymous direct callers leave it absent. Discovery covers both shapes: the service lists direct children and the full descendant tree — mode, activity, and lineage — reading live session state and optional persistence, without loading any child.
...
- **Browser continuation requires a live parent** — the `prompt` Remote requires the exact live direct parent; a durable human parent address alone cannot resume the child.
