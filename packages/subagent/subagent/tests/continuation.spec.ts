...
import type {
  AuthenticatedPrincipal, GenerateOptions, MessageId, StreamChunk,
...
describe('continuable review regressions', () => {
  ...
  it('clears the accepted reservation when Agent.followup throws', async () => {
    ...
    const rejectedPrincipal = {
      source: 'gateway', id: 'bob', username: 'bob', role: 'user' as const,
    }
    const started = await ctx.subagents.startContinuable(startSpec(
      parent, 'spawn', testSignal, acceptedPrincipal,
    ))
    await vi.waitFor(() => { expect(adapter.requests).toHaveLength(1) })
    const child = ctx.agents.get(started.childId)!
    const manager = (ctx.subagents as unknown as {
    ...
    await expect(followup(
      ctx, parent, started.childId, message('throws'), testSignal, rejectedPrincipal,
    ))
      .rejects.toThrow(/synthetic inbox failure/)
    expect(activation.accepted.size).toBe(0)
    expect(activation.principal).toEqual(acceptedPrincipal)
...
describe('continuable settlement delivery', () => {
  it('retains an accepted follow-up principal on the child message and settlement', async () => {
    const releaseFirst = Promise.withResolvers<undefined>()
    const adapter = new GatedAdapter([
      { chunks: textResponse('first answer'), gate: releaseFirst.promise },
    ...
    await followup(ctx, parent, started.childId, message('owned follow-up'), testSignal, principal)
    const child = ctx.agents.get(started.childId)
    expect(child?.inbox.nextTurn).toMatchObject([{ principal }])
    releaseFirst.resolve(undefined)
    await waitNoActivation(ctx, started.childId)

    const loaded = await ctx.sessionPersistence.load(started.childId)
    const accepted = loaded.events.find(event => event.type === 'user/message'
      && event.data.content.some(block => block.type === 'text' && block.text === 'owned follow-up'))
    expect(accepted).toMatchObject({ data: { principal } })
    await vi.waitFor(() => { expect(settlementNotices(parent)).toHaveLength(1) })
    const settlement = parent.session.events.find(event => event.type === 'user/message'
      && event.data.source.kind === 'subagent-settled')
    expect(settlement).toMatchObject({ data: { principal } })
  })
  ...
  it('tells the parent what the child finished with, without being asked', async () => {
    ...
    const started = await ctx.subagents.startContinuable(
      startSpec(parent, 'spawn', testSignal, principal),
    )
    await waitNoActivation(ctx, started.childId)
