...
需要所选提供方不具备的能力的请求会在启动时明确报错，而不会被静默忽略。失败的子 agent 运行会返回停止原因，提供方后端还会附加安全诊断；被取消的请求以 `aborted` 结算。浏览器目录读取与消息提交在父会话授权或请求操作期间被取消时，返回 `gateway/cancelled`。子 agent 相互隔离：崩溃或行为异常的子 agent 无法破坏父级会话。
