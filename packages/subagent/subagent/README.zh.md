...
可继续子 agent 把后续消息作为下一个轮次回答，父级随时可以中断运行中的轮次或列举自己的子级。浏览器 `prompt` Remote 会把传输层验证过的 Gateway principal 快照到已接受的子级消息与该 Activation 的最终结算通知；后续接受的新消息会替换结算归属者。匿名直接调用方仍不携带 principal。浏览器发出的继续执行 prompt 可以携带图片部分：Host 先通过附件存储完成整批图片的准入与持久化，子级 inbox 才接受这条消息；当子级声明的模型不接受图片输入时拒绝投递。发现覆盖两种形态：服务列举直接子级与完整后代树——模式、活动状态与血缘——直接读取在线会话状态与可选持久化，不加载任何子 agent。
...
- **浏览器继续执行要求在线父级**——`prompt` Remote 要求确切在线直接父级；仅凭持久化的人类父级地址无法恢复子级。
