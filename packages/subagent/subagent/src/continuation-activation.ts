...
import type { AuthenticatedPrincipal, MessageId } from '@deepseek-ai/dsh-llm'
import type {
  SessionEvent,
  SessionId,
...
export interface Activation {
   ...
   */
  announced: boolean
  /** Owner of the latest accepted child turn, carried onto child-generated parent messages. */
...
export class ContinuableActivationRegistry {
   ...
   * @param principal - authenticated owner recorded for this delivery and its later settlement.
  ...
  private notifySettlement(activation: Activation, terminal: ActivationTerminal): void {
    ...
    try {
      ...
      const message = createSettlementMessage(activation.childId, terminal, activation.principal)
