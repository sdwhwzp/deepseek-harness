...
export class SubagentRuntime extends TypertRemoteService {
   ...
   * validated browser zone on the accepted message. The transport-verified
   * caller is snapshotted from the active Gateway invocation. Success identifies the
  ...
  async prompt(request: SubagentPromptRequest, signal: AbortSignal): Promise<SubagentPromptReceipt> {
    ...
    const principal = this.ctx.get('typertGateway')?.currentPrincipal()
    try {
      ...
      return {
        messageId: await this.followup(parent, childSessionId, content, {
          source,
          signal,
