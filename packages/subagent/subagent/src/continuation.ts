...
export class SubagentContinuationManager {
   ...
   * @param principal - authenticated owner attached to the accepted prompt.
  ...
  private sendToParent(
    ...
    const message = createAgentMessage(sender, content, principal)
  ...
  private submitAdmitted(
    ...
    const message = options.source === undefined
      ? createAgentMessage(parent, content, options.principal)
      : createUserMessage({
        content,
