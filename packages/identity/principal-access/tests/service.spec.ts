...
describe('principal access seam', () => {
  ...
  it('delegates authenticated decisions and checks exact Session and Workspace grants', async () => {
    const ctx = new Context()
    ...
    expect(() => { requirePrincipalAccess(result, { kind: 'session', id: sessionId }) })
      .not.toThrow()
    expect(() => { requirePrincipalAccess(result, { kind: 'workspace', id: workspaceId }) })
      .not.toThrow()
  })
