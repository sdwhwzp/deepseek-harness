...
/**
 * A request that cannot inherit local-anonymous access or names a resource
 * omitted from the deployment provider's readable subset.
 */
...
/**
 * Service Definition for deployment authorization. Providers validate the
 * principal's source and subject against deployment-owned account data, then
 * return only the requested resource ids that principal may read.
 */
export abstract class PrincipalAccessService extends Service {
   ...
   * @param principal - Host-verified message-scoped identity.
...
/**
 * Resolve principal-scoped readability with the deployment's strict defaults.
 * A fully local request with no request-authentication provider, principal,
 * or access provider keeps legacy access. Any authenticated or half-configured
 * deployment fails closed unless both principal and access provider are
 * present; the access provider then decides the readable subset.
