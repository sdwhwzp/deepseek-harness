...
与 `ui-conversation` 及命令包一起挂载本插件；composer 随即在待处理指示器旁显示模型位，`/model` 则以弹窗打开同一份目录。当确切提供方／模型对仍在已公布分组中时，两个界面都显示Host 报告的当前选择；目录行缺席时，可路由的选择保持不变，触发器显示其提供方／模型 id。连接重置并重新加载目录期间，触发器的可见文字和无障碍标签均保留最后确认的模型与推理强度名称。
