...
describe('createFixtureApi commands/skills', () => {
  it('serves file references from the explicit Session wire field', async () => {
    const { rpc } = createFixtureFaces()
    const files = await callRemote<Array<{ path: string }>>(
      rpc,
      'fileReferences/list',
      { sessionId: sid('fx-alpha'), query: 'README' },
    )

    expect(files).toEqual([{ path: 'README.md', kind: 'file' }])
  })
