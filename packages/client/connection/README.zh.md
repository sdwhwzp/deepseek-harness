...
浏览器通过 HTTP POST 执行 Remote 一元调用；API Gateway 自己拥有 `/api/remote.mux` WebSocket 及其逻辑流。进程内组合通过 `connection.rpc.open` 提供等价的 Remote 流，不打开 WebSocket。Host half 拥有唯一 `/api` route、Fetch bridge、浏览器认证、Host/Origin 校验与精确 `GET`/`HEAD` 路由注册表。Typert Gateway 和其他 Host 功能注册可独立释放的 Remote interceptor。互不重叠的 endpoint matcher 可以共存；若多个 interceptor 认领同一 endpoint，Connection 会在任一 handler 运行前返回 500。精确 Fetch 路由保持更高优先级，未认领的请求返回 404。Loopback hostname 判定只供浏览器侧当前页面状态使用，留在包内。
...
浏览器认证通过后，可选的 Host `ctx.requestPrincipal` 提供方可以通过 `ctx.connection.authenticateRequest()` 认证由部署持有的请求数据；这是 HTTP Fetch 分发和 WebSocket upgrade 共用的入口。成功结果会在路由选择或 socket handoff 前经过校验、复制和冻结。精确 Fetch handler 通过第 2 个参数接收它，RPC handler 则通过第 4 个参数接收；浏览器 JSON 无法提供该值。提供方拒绝、失败或返回无效结果时，系统会在任何 endpoint 或 upgrade 运行前返回 401。省略提供方时，本地匿名运行方式保持不变。参见[消息作用域 principal](../../../.agents/notes/implemented/architecture/2026-08-21-message-scoped-authenticated-principals.zh.md)。
