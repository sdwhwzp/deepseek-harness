...
可选的 Host `ctx.requestPrincipal` 提供方会在共享 `/api` handler 选择 Typert interceptor 或 API Proxy 回退之前认证每条 Request。成功结果会经过校验、冻结，并且只附着到这一条进程内 Request；浏览器 JSON 中的身份字段一律忽略。同一 principal 会进入通用 Remote 分发与 `RpcRequest`；未配置提供方时保留匿名本地兼容。提供方拒绝会在任何端点运行前返回 401。基础 Web 载体本身仍不签发或校验凭据；远程部署通过已认证网关提供该服务。
