...
The browser uses HTTP POST for Remote unary calls. API Gateway owns the `/api/remote.mux` WebSocket and its logical streams; in-process compositions provide equivalent Remote streams through `connection.rpc.open` without opening a WebSocket. The Host half owns the sole `/api` route, Fetch bridge, browser authentication, Host/Origin checks, and exact `GET`/`HEAD` route registry. Typert Gateway and other Host features register independently disposable Remote interceptors. Disjoint endpoint matchers coexist; if multiple interceptors claim one endpoint, Connection returns 500 before either handler runs. Exact Fetch routes retain precedence, and unclaimed requests return 404. Loopback hostname classification remains package-internal to the browser-facing Client state.
...
After browser authentication, an optional Host `ctx.requestPrincipal` provider may authenticate deployment-owned request data through `ctx.connection.authenticateRequest()`, the shared entry for HTTP Fetch dispatch and WebSocket upgrades. A successful result is validated, copied, and frozen before route selection or socket handoff. Exact Fetch handlers receive it as their second argument and RPC handlers as their fourth; browser JSON cannot supply it. Provider rejection, failure, or an invalid result returns 401 before an endpoint or upgrade runs. Omitting the provider preserves anonymous local operation. See [message-scoped principals](../../../.agents/notes/implemented/architecture/2026-08-21-message-scoped-authenticated-principals.md).
