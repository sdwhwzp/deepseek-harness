...
Legacy RPC channels require a Web server at registration time. Connection resolves that optional service through `ctx.get` and binds route disposal to the registering plugin.
...
Every Host RPC method and WebSocket stream requires either a transport principal or one browser session; there is no method-specific loopback tier. An optional deployment `ctx.requestPrincipal` Service Provider authenticates the carrier-owned `Request` before route selection. A successful result is detached, validated, and frozen, then bound to an account-specific PeerScope for Remote handlers and WebSocket generations, and passed to exact Fetch routes. Separate account identities never share a PeerScope. Identity-looking browser JSON fields cannot create or replace it. A provider rejection or malformed result returns 401 before an endpoint runs; when the provider declines or is absent, Connection applies browser-session authentication.
