...
Every Host RPC method and WebSocket stream requires either a transport principal or one browser session; there is no method-specific loopback tier. An optional deployment `ctx.requestPrincipal` Service Provider authenticates the carrier-owned `Request` before route selection. A successful result is detached, validated, and frozen, then passed only as a Host-side argument to Remote handlers, exact Fetch routes, and the WebSocket generation opened by that request. Identity-looking browser JSON fields cannot create or replace it. A provider rejection or malformed result returns 401 before an endpoint runs; when the provider declines or is absent, Connection applies browser-session authentication.
