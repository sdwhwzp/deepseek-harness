...
/** Optional Host authentication provider used before browser request dispatch. */
...
export interface HostConnectionHandle {
  ...
  /**
   * Authenticate, validate, detach, and freeze one carrier request principal.
   * @param request - HTTP Fetch or upgrade request facts owned by the carrier.
   * @returns the verified caller, or undefined for an anonymous deployment.
   * @throws when the configured provider rejects, fails, or returns an invalid principal.
   */
  authenticateRequest(
    request: ConnectionPrincipalRequest,
