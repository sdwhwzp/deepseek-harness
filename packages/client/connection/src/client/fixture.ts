  ...
  // keyed registration a top-level bash row uses).
  {
    ...
    const dispatchPair = (n: number, name: string, dispatchArgs: Record<string, unknown>, resultText: string, isError = false): void => {
      push({
        ...
        data: { rootCallId: callId, rootCallSeq, parentCallId: callId, subCallId: `${callId}:ptc:${n}`, name, arguments: dispatchArgs },
      })
      push({
        type: 'tool/ptc-dispatch',
        data: {
          rootCallId: callId, rootCallSeq, parentCallId: callId, subCallId: `${callId}:ptc:${n}`, name,
