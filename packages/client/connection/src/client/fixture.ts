...
function buildAlphaLog(): SessionEvent[] {
  ...
  for (let turn = 0; turn < 60; turn++) {
    ...
    if (withTool) {
      ...
      push({
        ...
        data: { turn, step: 0, message: toolResultMessage(callId, text(`ECHO: TURN ${turn}`), turn % 25 === 12) },
      })
  ...
  // keyed registration a top-level bash row uses).
  {
    ...
    const rootCallSeq = push({ type: 'tool/call', data: { turn, step: 0, callId, name: 'run_code', arguments: args } })
    const dispatchPair = (n: number, name: string, dispatchArgs: Record<string, unknown>, resultText: string, isError = false): void => {
      push({
        type: 'tool/code-dispatch-start',
        data: { rootCallId: callId, rootCallSeq, parentCallId: callId, subCallId: `${callId}:code:${n}`, name, arguments: dispatchArgs },
      })
      push({
        type: 'tool/code-dispatch',
        data: {
          rootCallId: callId, rootCallSeq, parentCallId: callId, subCallId: `${callId}:code:${n}`, name,
    ...
    push({
      type: 'tool/result', sourceEventSeqs: [rootCallSeq], surfaceOp: 'append',
