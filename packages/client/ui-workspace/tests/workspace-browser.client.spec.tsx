...
describe('WorkspaceBrowser', () => {
  it('renders Workspace actions before the header and supplies the column width', () => {
    const owners: { wide: boolean }[] = []
    const renderSlot = ((name: string, owner: { open?: boolean; wide?: boolean }) => {
      if (name === 'sidebar.workspaces.action') {
        owners.push({ wide: owner.wide === true })
        return <div data-testid="workspace-action" />
      }
      return owner.open === true ? <div data-testid="directory-flow" /> : null
    }) as WorkspaceBrowserProps['renderSlot']
    const b = mount({ renderSlot })
    const action = screen.getByTestId('workspace-action')
    const header = screen.getByText('工作区')
    expect(action.compareDocumentPosition(header) & Node.DOCUMENT_POSITION_FOLLOWING).not.toBe(0)
    expect(owners.length).toBeGreaterThan(0)
    expect(owners.every(owner => owner.wide)).toBe(true)

    rerender(b, { wide: false })
    expect(owners.at(-1)).toEqual({ wide: false })
  })
