...
async function handlePresentOpen(
  ctx: Context, request: Request, principal: AuthenticatedPrincipal | undefined,
): Promise<Response> {
  ...
  try {
    ...
    if (error instanceof PrincipalAccessDeniedError) {
      const status = error.reason === 'principal-required' ? 401 : error.reason === 'provider-required' ? 503 : 404
      return new Response('Presented file unavailable.', { status })
    }
