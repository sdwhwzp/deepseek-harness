...
import { PrincipalAccessDeniedError, requirePrincipalAccess, resolvePrincipalAccess } from '@deepseek-ai/dsh-principal-access'
...
/** Host-verified caller identity the Connection fence binds to every deliverables request. */
type Principal = AuthenticatedPrincipal | undefined
 ...
 * and changed-file opening. Every Session-addressed route first proves the
 * caller may read that Session, so one account never opens or lists another
 * account's files.
...
/** Translate access refusals, lookup failures, and filesystem failures into the status the browser retries from. */
function failureStatus(error: unknown): number {
  if (error instanceof PrincipalAccessDeniedError) {
    return error.reason === 'principal-required' ? 401 : error.reason === 'provider-required' ? 503 : 404
  }
...
/**
 * Prove the caller may read the addressed Session.
 * @throws PrincipalAccessDeniedError when the deployment refuses the read.
 */
async function requireSessionRead(ctx: Context, request: Request, sessionId: SessionId, principal: Principal): Promise<void> {
  const access = await resolvePrincipalAccess(ctx, principal, { sessionIds: [sessionId] }, request.signal)
  requirePrincipalAccess(access, { kind: 'session', id: sessionId })
}

/**
 * Read the addressed event once the Host desktop is known to be available and the caller may read the Session.
 * @returns the event with its Session header, or the refusal to answer with.
 */
async function readTarget(ctx: Context, request: Request, id: string, seq: number, principal: Principal): Promise<Awaited<ReturnType<Context['sessionQuery']['readEvent']>> | Response> {
  request.signal.throwIfAborted()
  if (!ctx.sessionController.workspaceDesktop().available) return new Response('Host desktop unavailable.', { status: 409 })
  await requireSessionRead(ctx, request, id as SessionId, principal)
  return ctx.sessionQuery.readEvent({ sessionId: id as SessionId, seq: seq as SessionSeq, before: 0, after: 0 }, request.signal)
}

/**
 * Open one verified Host path on behalf of the caller. A file is verified
 * through the Session filesystem; a directory is verified by the Host
 * filesystem mapping alone. The open goes through the typert gateway so the
 * Session controller decides it against the same principal.
 * @returns the HTTP status to answer with.
 */
async function openVerified(ctx: Context, request: Request, sessionId: SessionId, path: string, action: 'open' | 'reveal', principal: Principal): Promise<Response> {
  const { fs } = ctx
...
async function handlePresentOpen(ctx: Context, request: Request, principal: Principal): Promise<Response> {
  ...
  try {
    const read = await readTarget(ctx, request, id, seq, principal)
    ...
    return await openVerified(ctx, request, id as SessionId, path, action, principal)
  } catch (error: unknown) {
    request.signal.throwIfAborted()
...
async function handleChangesSummary(ctx: Context, request: Request, principal: Principal): Promise<Response> {
  const query = new URL(request.url).searchParams
  const id = query.get('sessionId')
  const seq = coordinate(query.get('seq'))
  ...
  try {
    ...
    request.signal.throwIfAborted()
    return new Response('Change summary unavailable.', { status: failureStatus(error) })
  }
...
async function handleChangesDiff(ctx: Context, request: Request, principal: Principal): Promise<Response> {
  ...
  try {
    await requireSessionRead(ctx, request, id, principal)
