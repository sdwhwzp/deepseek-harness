...
async function fixture() {
  ...
  // The route opens through the Gateway so the Session controller decides against the caller's principal.
  ...
  await ctx.plugin({
    inject: ['connection', 'sessionQuery', 'sessionController', 'typertGateway', 'workspaceFiles', 'fs', 'sandboxPolicy', 'workspaceChanges'],
...
describe('changed files native open route', () => {
  it('opens a listed file inside or outside the workspace with its verified Host path', async () => {
    ...
    expect(response.headers.get('cache-control')).toBe('no-store')
    expect(opener).toHaveBeenLastCalledWith({ sessionId: SessionId('owner'), path: await realpath(join(cwd, 'src', 'lib', 'a.ts')) }, expect.any(AbortSignal))
