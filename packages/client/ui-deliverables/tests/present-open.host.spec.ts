...
async function fixture() {
  ...
  const opener = vi.fn(async (_request: { sessionId: SessionId; path: string; action?: 'reveal' }, _signal: AbortSignal) => ({ opened: true as const }))
  ...
  const invoke = vi.fn(async (request: InvokeRemoteRequest) => request.method === 'workspacePathApplications' ? applications() : opener(
    request.args['request'] as { sessionId: SessionId; path: string; action?: 'reveal' },
    request.signal!,
  ))
  ctx.provide('typertGateway', { invoke } as never)
  const connection = new HostConnectionService(ctx, [], {} as BrowserAuth)
  const fiber = ctx.plugin({ inject: ['connection', 'sessionQuery', 'sessionController', 'typertGateway', 'workspaceFiles', 'fs', 'sandboxPolicy'], apply: registerPresentOpen })
  await fiber
  ...
  return { applications, root, cwd, ctx, fiber, file, session, readEvent, open, opener, handler, resolveAgent, invoke }
}
...
it('reports the serving desktop and reveals only an authorized declared source', async () => {
  ...
  expect(opener).toHaveBeenCalledWith({ sessionId: 'owner', path: await realpath(join(cwd, file.path)), action: 'reveal' }, expect.any(AbortSignal))
...
it('authorizes the viewed Session before reading and carries its verified principal into native opening', async () => {
  const { ctx, open, readEvent, invoke } = await fixture()
  const principal: AuthenticatedPrincipal = { source: 'gateway', id: 'alice', username: 'Alice', role: 'user' }
  ctx.provide('requestPrincipal', { authenticate: () => principal })
  const resolve = vi.fn(async () => ({ readableSessionIds: new Set<SessionId>(), readableWorkspaceIds: new Set() }))
  ctx.provide('principalAccess', { resolve } as never)
  expect((await open()).status).toBe(404)
  expect(readEvent).not.toHaveBeenCalled()
  expect(invoke).not.toHaveBeenCalled()
  resolve.mockResolvedValue({ readableSessionIds: new Set([SessionId('owner')]), readableWorkspaceIds: new Set() })
  expect((await open()).status).toBe(204)
  expect(resolve).toHaveBeenLastCalledWith(principal, { sessionIds: ['owner'] }, expect.any(AbortSignal))
  expect(invoke).toHaveBeenCalledWith(expect.objectContaining({ principal, namespace: 'session', method: 'openWorkspacePath' }))
})

it('fails closed when authenticated native opening lacks its deployment provider or identity', async () => {
  const { ctx, open, readEvent } = await fixture()
  ctx.provide('requestPrincipal', { authenticate: () => undefined })
  expect((await open()).status).toBe(503)
  ctx.provide('principalAccess', { resolve: vi.fn() } as never)
