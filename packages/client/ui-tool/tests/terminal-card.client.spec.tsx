...
describe('DetailsPanel Output section', () => {
  ...
  it('renders the selected lifecycle when two roots reuse one call id', () => {
    const first = settled({
      call: { name: 'bash', argsRaw: shellArgs({ command: 'first command' }) },
      content: [{ type: 'text', text: 'FIRST_LIFECYCLE_OUTPUT' }],
    })
    const second = settled({
      seq: 12,
      call: { name: 'bash', argsRaw: shellArgs({ command: 'second command' }) },
      content: [{ type: 'text', text: 'SECOND_LIFECYCLE_OUTPUT' }],
    })
    const view = mount(snapshot({ nodes: [first, second] }), {
      turnSeq: 10,
      toolCall: { nodeKey: 'tool:c1:10', rootCallSeq: 10, callId: 'c1' },
      toolName: 'bash',
    })

    expect(view.container.querySelector('pre code')?.textContent)
      .toContain('"command": "first command"')
    expect(view.getByText('FIRST_LIFECYCLE_OUTPUT')).toBeTruthy()
    expect(view.queryByText(/second command/)).toBeNull()
    expect(view.queryByText('SECOND_LIFECYCLE_OUTPUT')).toBeNull()
  })
