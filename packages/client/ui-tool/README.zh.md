...
专用 `read_image` 视图会在可展开的 read-family 卡片中渲染通过校验的图片结果。其他每个已结算 root 或嵌套工具结果都会把标准图片块以靠前端对齐方式显示在所属行正下方。两条路径都保留 conversation 所属、按会话授权的附件 loader、按会话 object URL 缓存、重试行为和共享原图灯箱。运行中的调用和不含图片的已结算结果不会添加画廊。
