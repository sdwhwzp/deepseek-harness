...
每个已完成的 root 或嵌套结果都通过 conversation 自有的消息图片 renderer，把标准图片块渲染在所属工具行下方。该 renderer 继续负责会话授权加载与共享原图灯箱；本包只提供持久引用和 `start` 对齐。运行中的调用与不含图片的结果不添加 gallery，`read_image` 使用普通 read 行展示。
...
`ToolCallTree` 接收一个已经包含递归 `subCalls` 的 root `ToolCallBlock`、selection 状态、会话 `cwd`，以及用于打开文件和检查调用的 Host 回调。精确 selection 组合 root Conversation Node key、root call seq 与原子 call id；Inspect 会把 root seq 和原子 id 转交给 trajectory。它递归遍历标准调用块，让 root 与任意深度的 child 经过同一条原子分发路径，不订阅独立的 parent-to-children map。每个 root 和 child 包装层都会发布由 root Node key 与原子 call id 派生的无冲突 `data-chat-anchor-key`，同时保留 `data-chat-call-id` 供 call-id 查询。
