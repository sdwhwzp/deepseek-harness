...
The dedicated `read_image` view renders a validated image result in its expandable read-family card. Every other settled root or nested Tool result renders its standard image blocks immediately below the owning row with leading alignment. Both paths retain the conversation-owned, session-authorized attachment loader, per-session object-URL cache, retry behavior, and shared original-image lightbox. Running calls and settled results without images add no gallery.
