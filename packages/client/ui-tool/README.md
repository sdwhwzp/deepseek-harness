...
Every settled root or nested result renders its standard image blocks below the owning Tool row through the conversation-owned message-image renderer. That renderer retains session-authorized loading and the shared original-image lightbox; this package supplies the durable references and `start` alignment. Running calls and results without images add no gallery, and `read_image` uses the ordinary read-row presentation.
...
`ToolCallTree` receives one root `ToolCallBlock` that already contains recursive `subCalls`, selection state, the session `cwd`, and Host callbacks for opening files and inspecting calls. An exact selection combines the root Conversation Node key, root call seq, and atomic call id; Inspect forwards the root seq and atomic id to trajectory. The tree recursively walks the standard call blocks and sends the root and children at every depth through the same atomic dispatch path, without subscribing to a separate parent-to-children map. Each root and child wrapper publishes a collision-free `data-chat-anchor-key` derived from the root Node key and atomic call id while retaining `data-chat-call-id` for call-id queries.
