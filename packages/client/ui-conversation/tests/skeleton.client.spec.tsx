...
describe('ConversationRoot resident composer', () => {
  ...
  it('renders bootstrap actions in the Hero control row after the agent-preset seat', () => {
    const b = mount(
      sessionSnapshotOf({ blank: true }),
      [{ ...workspace('one'), sessionIds: [SID] }, { ...workspace('second'), title: 'Selected Folder' }],
    )
    expect(b.view.container.querySelector('[data-hero-controls]')).not.toBeNull()
    const preset = b.slotCalls.indexOf('conversation.hero.agentPreset')
    const bootstrap = b.slotCalls.indexOf('conversation.input.bootstrap')
    const composer = b.slotCalls.indexOf('conversation.composer.bar')
    expect(preset).toBeGreaterThanOrEqual(0)
    expect(bootstrap).toBeGreaterThan(preset)
    expect(composer).toBeGreaterThan(bootstrap)
  })

  it('omits root bootstrap actions from an active-session composer', () => {
    const b = mount(sessionSnapshotOf({ blank: false }))
    expect(b.view.queryByTestId('hero-headline')).toBeNull()
    // The Hero control row (and the bootstrap seat inside it) is not mounted
    // for an active transcript; slot calls alone cannot prove that because
    // the row's elements are built before the phase decides to mount them.
    expect(b.view.container.querySelector('[data-phase]')?.getAttribute('data-phase')).toBe('active')
    expect(b.view.container.querySelector('[data-hero-controls]')).toBeNull()
  })
