...
export class ConversationNodeAssembler implements ConversationViewSnapshotStore {
  ...
  private dispatchInput(
    ...
    for (const definition of this.eventDefinitions.entries()) {
      ...
      publication = maximumPublication(
        publication,
        accept(definition, result.id, result.lifecycle, result.role),
      )
    }
    ...
    if (fallback !== undefined && target !== undefined && !matchedTargets.has(target)) {
      ...
      if (result !== null) {
        publication = maximumPublication(
          publication,
          accept(fallback, result.id, result.lifecycle, result.role),
        )
      }
    }
    return publication
  ...
  private acceptMatch(
    ...
    context ??= this.createContext(definition, id, key, lifecycle)
    const match = conversationMatch(
      key,
      input,
      role,
  ...
  private applyPendingMatches(
    ...
    for (const [key, entries] of pending) {
      ...
      let context = this.contexts.get(key)
      context ??= this.createContext(first.definition, first.id, key, first.lifecycle)
