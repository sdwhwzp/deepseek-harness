...
Before creating the submission echo or admitting a prompt, `conversation/prepare-send` awaits client setup listeners in registration order. The blank-session view stays visible while setup is pending. A listener rejection prevents both the echo and prompt request and lets the composer restore the draft. Both scoped `send` and composer submissions use this event.
...
View selection is deterministic: a registered persisted selection wins, otherwise registered `chat` wins, otherwise no View renders. It never chooses the first registered View. Transcript width handles operate only in the Chat view; their indicators appear during a drag and stay hidden on hover. Shell phase combines Session lifecycle with the active-target set; no target-specific snapshot is read by the shell.
