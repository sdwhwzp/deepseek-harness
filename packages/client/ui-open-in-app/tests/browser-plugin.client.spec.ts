...
describe('open-in-app browser half', () => {
  ...
  it('registers both document-preview path controls behind one desktop answer, and fiber teardown removes them', async () => {
    ...
    if (!header?.inject || !empty?.inject) throw new Error('Missing registered path controls')
    const face = Reflect.apply(header.inject, undefined, [SessionId('owner')]) as OpenPathInjected
    const emptyFace = Reflect.apply(empty.inject, undefined, [SessionId('owner')]) as OpenPathInjected
    ...
    expect(remote.session.workspacePathApplications).toHaveBeenCalledWith({ sessionId: SessionId('owner'), path: '/w/clip.mp4' }, signal)
    expect(face.hooks.openInAppDesktop.getSnapshot()).toBe(true)
    expect(await face.openPath('/w/clip.mp4', 'reveal')).toBeNull()
    expect(remote.session.openWorkspacePath).toHaveBeenLastCalledWith({ sessionId: SessionId('owner'), path: '/w/clip.mp4', action: 'reveal' })
    await fiber.dispose()
