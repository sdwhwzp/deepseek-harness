...
export interface OpenInAppPathRemote {
  readonly session: {
    ...
    workspacePathApplications(
      request: { sessionId: SessionId; path: string }, signal?: AbortSignal,
...
export class OpenInAppPathController {
  ...
  /**
   * Open one Host path in its default application, or reveal it in the file manager.
   * @param sessionId - Session authorizing the file access.
   ...
   */
  async openPath(
    sessionId: SessionId, path: string, action: OpenInAppPathAction, application?: string,
  ): Promise<OpenInAppPathFailure | null> {
    const request: SessionOpenWorkspacePathRequest = action === 'reveal' ? { sessionId, path, action } : { sessionId, path, ...(application === undefined ? {} : { application }) }
    let ok = false
    try {
   ...
   */
  async applications(sessionId: SessionId, path: string, signal: AbortSignal): Promise<readonly SessionWorkspacePathApplication[] | null> {
    let result: RemoteResult<readonly SessionWorkspacePathApplication[]>
    try {
      result = await this.remote.session.workspacePathApplications({ sessionId, path }, signal)
    } catch (_error) {
