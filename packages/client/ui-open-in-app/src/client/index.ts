...
export function apply(ctx: ClientContext): void {
  ...
  const pathInjected = (sessionId: SessionId): OpenPathInjected => ({
    hooks: { openInAppDesktop: paths.desktop },
    loadDesktop: () => paths.load(),
    openPath: (path, action, application) => paths.openPath(sessionId, path, action, application),
    applications: (path, signal) => paths.applications(sessionId, path, signal),
  })
