...
import type {
  ...
  ToolCallBlock, ToolCallTarget, ToolResultNode, TurnErrorNode, TurnMaxTokensNode, UseChatNodeTurnData,
...
function makeHarness(
  ...
  const toolOwners: Array<{
    ...
    toolName: string
    block: ToolCallBlock
    selectedToolCall: ToolCallTarget | undefined
    ...
    switch (nodeOwner.node.kind) {
      ...
      case 'tool-call': {
        ...
        const tool = {
          ...
          toolName,
          block,
          selectedToolCall: nodeOwner.selectedToolCall,
...
describe('ChatView', () => {
  ...
  it('hands the trajectory callback to the Tool seat', () => {
    ...
    h.toolOwners[0]?.inspectCall({ nodeKey: 'fixture:tool:a', rootCallSeq: 3, callId: 'a' })
    expect(h.openView).toHaveBeenCalledWith(
      'trajectory',
      '{"callId":"a","rootCallSeq":3}',
    )
  })
  ...
  it('updates the selected Tool lifecycle handed to the Tool seat', () => {
    const h = makeHarness({ nodes: [toolResult(3, 'a')] })
    render(<h.ChatView {...h.props} />)
    expect(h.toolOwners.at(-1)?.selectedToolCall).toBeUndefined()
    const toolCall = { nodeKey: 'fixture:tool:a', rootCallSeq: 3, callId: 'a' }
    act(() => { h.setSelection({ turnSeq: 3, toolCall, toolName: 'bash' }) })
    expect(h.toolOwners.at(-1)?.selectedToolCall).toEqual(toolCall)
  })
  ...
  it('hands each ordered root call to the keyed business-node slot', () => {
    ...
    expect(calls[0]).toMatchObject({
      ...
      owner: { node: { kind: 'tool-call' }, selectedToolCall: undefined },
      entryKey: 'tool-call',
    })
    ...
    owner.inspectCall({ nodeKey: 'fixture:tool:a', rootCallSeq: 3, callId: 'a' })
    expect(h.openView).toHaveBeenCalledWith(
      'trajectory',
      '{"callId":"a","rootCallSeq":3}',
    )
