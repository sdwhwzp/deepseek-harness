...
<a id="attempt-recovery"></a>
## Attempt recovery

Chat removes the temporary output of a failed or abandoned model attempt before displaying a retry. A displayed Assistant node and its process control retain their identities while hidden, so the next reply and completed-turn state appear without reloading the page.
