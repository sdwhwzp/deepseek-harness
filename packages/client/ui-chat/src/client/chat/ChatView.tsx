...
/** Encode the lifecycle identity consumed by the trajectory view. */
function trajectoryToolFocus(target: ToolCallTarget): string {
  return JSON.stringify({ callId: target.callId, rootCallSeq: target.rootCallSeq })
}
...
export function ChatView({
  ...
  const selectedToolCall = useStore(s => s.selection?.toolCall)
  const compactTranscript = useTranscriptView(mode => mode === 'compact')
  const inspectCall = useCallback((target: ToolCallTarget) => {
    openView('trajectory', trajectoryToolFocus(target))
  }, [openView])
