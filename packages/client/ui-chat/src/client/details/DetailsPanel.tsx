...
import { findLatestToolCall, findToolCall } from './tool-node-reader.ts'
...
function materialFor(
  s: ChatSnapshot,
  callId: string,
  target: ToolCallTarget | undefined,
): CallMaterial | null {
  const found = target === undefined
    ? findLatestToolCall(s, callId)
    : findToolCall(s, target)
...
export function DetailsPanel({ useChat, useSessions, sessionId, useStore, renderSlot, closeDetails, t }: DetailsPanelProps) {
  ...
  const target = selection?.toolCall
  const callId = target?.callId ?? selection?.callId
  // materialFor builds a fresh wrapper; shallowEqual short-circuits on its
  // stable members (result node reference rides the snapshot's structural sharing).
  const material = useChat(
    s => (callId === undefined ? null : materialFor(s, callId, target)),
            ...
            : (
              <>
                ...
                <section className={css.section}>
                  ...
                  <Fragment key={target === undefined ? callId : `${target.nodeKey}\u0000${callId}`}>
