...
客户端在每次读取候选项时，都会本地化已知的 `compact`、`export`、`feedback`、`goal`、`image`、`permission`、`plan` 与 `read-image` 宿主命令说明。每项映射会同时核对命令名与规范英文说明，因此使用相同名称的作用域命令仍保留自身文案。可选的 `image` 与 `read-image` 行仅在对应宿主命令已注册时出现；未知宿主命令保留宿主提供的说明，客户端贡献项则自行管理本地化文案。
