...
import type {
  AssistantMetricDetail, TrajectoryCallFocus, TrajectoryCellKind, TrajectoryCellProps,
...
export interface TrajectoryTableProps {
  ...
  inspectCall?: TrajectoryCallFocus | null
...
function SourceBlocks({
  ...
  onOpenCall: (focus: TrajectoryCallFocus) => void
  renderImages: RenderMessageImages
  t: TrajectoryTranslate
}) {
  return (
              ...
              <button
                ...
                onClick={() => {
                  if (block.callId !== undefined) {
                    onOpenCall({ callId: block.callId, rootCallSeq: block.rootCallSeq ?? null })
                  }
                }}
              >
  ...
  return (
    ...
    >
      {calls.map((call, index) => (
        <li key={call.callId === undefined
          ? index
          : `${call.rootCallSeq ?? 'legacy'}\u0000${call.callId}`}>
          <button
            type="button"
            ...
            onClick={() => {
              if (call.callId !== undefined) {
                onOpenCall({ callId: call.callId, rootCallSeq: call.rootCallSeq ?? null })
              }
            }}
          >
            <svg
...
export function TrajectoryTable({
  ...
  const openCallSummary = (focus: TrajectoryCallFocus) => {
    const target = allRecords.find(record => record.cell.callId === focus.callId
      && (focus.rootCallSeq === null || record.cell.rootCallSeq === focus.rootCallSeq))
  ...
  useEffect(() => {
    if (inspectCall === null) return
    const target = flattenRecords(turns).find(record => record.cell.callId === inspectCall.callId
      && (inspectCall.rootCallSeq === null
        || record.cell.rootCallSeq === inspectCall.rootCallSeq))
    if (target === undefined) return
    openRecordSummaryRef.current(target)
    pendingScrollRecordId.current = trajectoryRecordId(target.cell)
    onInspectApplied?.()
  }, [inspectCall, turns, onInspectApplied])
  useEffect(() => {
