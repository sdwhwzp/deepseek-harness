...
import type { TrajectorySnapshot, TrajectoryToolLifecycle } from './trajectory-contract.ts'
import { formatElapsedSeconds, trajectoryToolKey } from './trajectory-record.ts'
...
export interface TrajectoryLayoutInput {
  ...
  toolLifecycles?: TrajectorySnapshot['toolLifecycles']
...
interface ToolLifecycleResolver {
  claim(callId: string, turn: number, step: number): TrajectoryToolLifecycle | undefined
  forResult(seq: number): TrajectoryToolLifecycle | undefined
  isClaimed(lifecycle: TrajectoryToolLifecycle): boolean
  unclaimedRunning(): readonly TrajectoryToolLifecycle[]
}
...
function legacyToolLifecycles(
  nodes: TrajectorySnapshot['eventNodes'],
  runningCalls: TrajectorySnapshot['runningCalls'],
): readonly TrajectoryToolLifecycle[] {
  return [
    ...nodes.flatMap(node => node.kind === 'tool-result'
      ? [{ rootCallSeq: null, turn: -1, step: -1, root: node }]
      : []),
    ...runningCalls.map(root => ({
      rootCallSeq: null,
      turn: root.turn,
      step: root.step,
      root,
...
function createToolLifecycleResolver(
  lifecycles: readonly TrajectoryToolLifecycle[],
): ToolLifecycleResolver {
  const byCallId = new Map<string, TrajectoryToolLifecycle[]>()
  const byResultSeq = new Map<number, TrajectoryToolLifecycle>()
  for (const lifecycle of lifecycles) {
    const calls = byCallId.get(lifecycle.root.callId) ?? []
    calls.push(lifecycle)
    byCallId.set(lifecycle.root.callId, calls)
    if ('kind' in lifecycle.root) byResultSeq.set(lifecycle.root.seq, lifecycle)
  }
  for (const calls of byCallId.values()) {
    calls.sort((left, right) =>
      (left.rootCallSeq ?? ('kind' in left.root ? left.root.seq : Number.MAX_SAFE_INTEGER))
      - (right.rootCallSeq ?? ('kind' in right.root ? right.root.seq : Number.MAX_SAFE_INTEGER)),
    )
  }
  const claimed = new Set<TrajectoryToolLifecycle>()
  return {
    claim: (callId, turn, step) => {
      const available = (byCallId.get(callId) ?? []).filter(lifecycle => !claimed.has(lifecycle))
      const exact = available.find(lifecycle => lifecycle.turn === turn && lifecycle.step === step)
      const legacy = available.find(lifecycle => lifecycle.turn < 0 || lifecycle.step < 0)
      const lifecycle = exact ?? legacy
      if (lifecycle !== undefined) claimed.add(lifecycle)
      return lifecycle
    },
    forResult: seq => byResultSeq.get(seq),
    isClaimed: lifecycle => claimed.has(lifecycle),
    unclaimedRunning: () => lifecycles.filter(lifecycle =>
      !claimed.has(lifecycle) && !('kind' in lifecycle.root),
    ),
  }
}
...
export function deriveTrajectoryLayout(
  ...
  const {
    ...
    toolLifecycles = legacyToolLifecycles(nodes, runningCalls),
  } = input
  const toolResolver = createToolLifecycleResolver(toolLifecycles)
  ...
  const entries: OrderedLayoutEntry[] = [
    ...nodes.map((node, nodeIndex) => ({
      kind: 'node' as const,
      seq: node.kind === 'tool-result'
        ? toolResolver.forResult(node.seq)?.rootCallSeq ?? node.seq
        : node.seq,
      node,
      nodeIndex,
    })),
  ...
  for (const entry of entries) {
    ...
    if (node.kind === 'assistant') {
      const laidList = withSubCalls(
        expandAssistant(node, index + 1, prevAbsTime, toolResolver, t),
        t,
      )
    ...
    if (node.kind === 'tool-result') {
      const lifecycle = toolResolver.forResult(node.seq)
      if (lifecycle === undefined || !toolResolver.isClaimed(lifecycle)) {
        ...
        const laidList: LaidCell[] = [{
          ...
          callId: node.callId,
          ...(lifecycle === undefined ? {} : { rootCallSeq: lifecycle.rootCallSeq }),
        ...
        for (const laid of expandSubCalls(node.subCalls, index, t, lifecycle?.rootCallSeq)) {
          laidList.push(laid)
  ...
  for (const lifecycle of toolResolver.unclaimedRunning()) {
    const call = lifecycle.root
    if ('kind' in call) continue
    const laidList: LaidCell[] = [{
      absTime: null,
      toolName: call.name,
      callId: call.callId,
      rootCallSeq: lifecycle.rootCallSeq,
    ...
    for (const laid of expandSubCalls(call.subCalls, index, t, lifecycle.rootCallSeq)) {
      laidList.push(laid)
      index = laid.cell.index
    }
 ...
 * @param t - Trajectory locale translator.
 * @param toolLifecycles - Root lifecycles available to the streaming prefix.
...
export function appendTrajectoryPartialLayout(
  ...
  lastIndex: number,
  t: TrajectoryTranslate,
  toolLifecycles: readonly TrajectoryToolLifecycle[] = [],
): readonly TrajectoryTurnModel[] {
  if (partial === null) return turns
  const settledRootSeqs = new Set(turns.flatMap(turn => turn.groups.flatMap(group =>
    group.cells.flatMap(cell => cell.rootCallSeq === undefined || cell.rootCallSeq === null
      ? []
      : [cell.rootCallSeq]),
  )))
  const partialTurn = deriveTrajectoryLayout({
    ...
    partial,
    runningCalls: [],
    toolLifecycles: toolLifecycles.filter(lifecycle =>
      !('kind' in lifecycle.root)
      || lifecycle.rootCallSeq === null
      || !settledRootSeqs.has(lifecycle.rootCallSeq)),
  }, t).at(0)
  ...
  for (const streamedGroup of streamed.groups) {
    ...
    const streamedCallKeys = new Set(
      streamedGroup.cells.flatMap((cell) => {
        const key = toolCellKey(cell)
    ...
    groups[groupIndex] = {
      ...
      cells: [
        ...group.cells.filter((cell) => {
          const key = toolCellKey(cell)
          return cell.requestOnly !== true
            && (key === undefined || !streamedCallKeys.has(key))
        }),
        ...streamedGroup.cells,
      ],
    }
...
function toolCellKey(cell: TrajectoryCellProps): string | undefined {
  if (cell.callId === undefined) return undefined
  return cell.rootCallSeq === undefined || cell.rootCallSeq === null
    ? `legacy\u0000${cell.callId}`
    : trajectoryToolKey(cell.callId, cell.rootCallSeq)
}
...
function attachToolSchema(
  ...
  if (laid.callId === undefined || laid.rootCallSeq === undefined
    || laid.rootCallSeq === null || callSchemas === undefined) return
  const schema = callSchemas.get(trajectoryToolKey(laid.callId, laid.rootCallSeq))
...
function expandAssistant(
  ...
  const toolLifecycles = node.blocks.map(block => block.kind === 'tool-call'
    ? tools.claim(block.callId, node.turn, node.step)
    : undefined)
  ...
  const message: TrajectoryCellProps = {
    ...
    sourceBlocks: node.blocks.map((block, blockIndex) =>
      assistantSourceBlock(block, toolLifecycles[blockIndex]?.rootCallSeq ?? null)),
  ...
  for (const [blockIndex, block] of node.blocks.entries()) {
    ...
    const lifecycle = toolLifecycles[blockIndex]
    const root = lifecycle?.root
    const result = root !== undefined && 'kind' in root ? root : undefined
    const toolDuration = streaming || result === undefined
      ? null
      : durationSeconds(result.time, result.callTime)
    const callAbs = root === undefined
      ? null
      : finiteTime('kind' in root ? root.callTime : root.time)
    ...
    out.push({
      ...
      callId: block.callId,
      ...(lifecycle === undefined ? {} : { rootCallSeq: lifecycle.rootCallSeq }),
      ...(root === undefined ? {} : { subCalls: root.subCalls }),
      cell: {
...
function withSubCalls(laidList: LaidCell[], t: TrajectoryTranslate): LaidCell[] {
  ...
  for (const laid of laidList) {
    ...
    for (const sub of expandSubCalls(laid.subCalls, index, t, laid.rootCallSeq)) {
      out.push(sub)
...
function expandSubCalls(
  ...
  for (const sub of subs) {
    ...
    out.push(laid)
    for (const child of expandSubCalls(sub.subCalls, index, t, rootCallSeq)) {
      out.push(child)
