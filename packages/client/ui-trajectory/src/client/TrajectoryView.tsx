...
import { decodeTrajectoryCallFocus, trajectoryRecordId } from './trajectory-record.ts'
...
export function TrajectoryView({
  ...
  const toolLifecycles = inspection.toolLifecycles
  const inspectCall = useMemo(() => viewRequest?.view === 'trajectory'
    ? decodeTrajectoryCallFocus(viewRequest.focus)
    : null, [viewRequest])
  ...
  const finalized = useMemo(() => {
    ...
    runningCalls, toolLifecycles, requests, callSchemas, t,
  ])
  ...
  const timelineTurns = useMemo(
    () => appendTrajectoryPartialLayout(
      finalized.turns, timelinePartial, finalized.lastIndex, t, toolLifecycles,
    ),
    [finalized, timelinePartial, t, toolLifecycles],
  )
  ...
  const partialSearchTurns = useMemo(
    () => appendTrajectoryPartialLayout([], partial, finalized.lastIndex, t, toolLifecycles),
    [finalized.lastIndex, partial, t, toolLifecycles],
  )
