...
describe('Trajectory conversation Definitions', () => {
  ...
  it('folds chained replacements onto the cited repeated-id lifecycle', () => {
    const resultMessage = (id: string, text: string) => ({
      id,
      role: 'user',
      source: { kind: 'tool', callId: 'reused' },
      content: [{
        type: 'tool-result',
        toolCallId: 'reused',
        content: [{ type: 'text', text }],
    ...
    const current = snapshot(assembler([
      ...
      at(2, 'step/start', { turn: 1, step: 1 }),
      at(3, 'tool/call', {
        turn: 1, step: 1, callId: 'reused', name: 'bash', arguments: '{"command":"pwd"}',
      }),
      at(4, 'tool/result', {
        turn: 1, step: 1, message: resultMessage('original', 'full output'),
      }, { time: 10_000, surfaceOp: 'append', sourceEventSeqs: [3] }),
      at(5, 'tool/call', {
        turn: 1, step: 1, callId: 'reused', name: 'bash', arguments: '{"command":"date"}',
      }),
      at(6, 'tool/result', {
        turn: 1, step: 1, message: resultMessage('second', 'second output'),
      }, { time: 20_000, surfaceOp: 'append', sourceEventSeqs: [5] }),
      at(7, 'compaction/prune', {
        shadowedRange: { start: 4, end: 4 }, shadowedSeqs: [4], shadowedTokenCount: 10,
      }),
      at(8, 'tool/result', {
        turn: 1, step: 1, message: resultMessage('replacement', 'pruned output'),
      }, {
        time: 30_000,
      ...
      at(9, 'tool/result', {
        turn: 1, step: 1, message: resultMessage('replacement-2', 'more pruned output'),
      }, {
        time: 40_000,
    ...
    expect(current.toolLifecycles).toHaveLength(2)
    expect(current.toolLifecycles[0]).toMatchObject({
      rootCallSeq: 3,
      turn: 1,
      step: 1,
      root: {
        ...
        content: [{ type: 'text', text: 'more pruned output' }],
      },
    })
    expect(current.toolLifecycles[1]).toMatchObject({
      rootCallSeq: 5,
      root: {
        kind: 'tool-result',
        seq: 6,
        time: 20_000,
        callId: 'reused',
        call: { name: 'bash', argsRaw: '{"command":"date"}' },
        content: [{ type: 'text', text: 'second output' }],
      },
    })
    expect(current.eventNodes.filter(node => node.kind === 'tool-result')).toHaveLength(2)
  })
