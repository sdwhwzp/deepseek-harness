...
describe('deriveTrajectoryLayout', () => {
  ...
  it('pairs repeated call ids with their lifecycle-local results and start times', () => {
    const assistant = {
      ...
      step: 1,
      blocks: [
        { kind: 'tool-call' as const, callId: 'same', name: 'bash', argsRaw: '{"command":"first"}' },
        { kind: 'tool-call' as const, callId: 'same', name: 'bash', argsRaw: '{"command":"second"}' },
      ],
    }
    const result = (seq: number, text: string, callTime: number) => ({
    ...
    const first = result(4, 'first output', 2_500)
    const second = result(7, 'second output', 6_000)
    const turns = deriveTrajectoryLayout({
      nodes: [assistant, first, second],
      partial: null,
      runningCalls: [],
      toolLifecycles: [
        { rootCallSeq: 3, turn: 1, step: 1, root: first },
        { rootCallSeq: 6, turn: 1, step: 1, root: second },
      ],
    })
    ...
    expect(tools).toMatchObject([
      { rootCallSeq: 3, startedAt: 2_500, outputDetail: 'first output' },
      { rootCallSeq: 6, startedAt: 6_000, outputDetail: 'second output' },
    ])
    expect(tools.map(trajectoryRecordId)).toEqual([
      'tool\u0000call\u00003\u0000same',
      'tool\u0000call\u00006\u0000same',
    ])
    const message = cells.find(cell => cell.kind === 'message')
    expect(message?.sourceBlocks).toMatchObject([
      { callId: 'same', rootCallSeq: 3 },
      { callId: 'same', rootCallSeq: 6 },
    ])
  })

  it('keeps a reused running call separate from the settled lifecycle', () => {
    const settled = {
      kind: 'tool-result' as const,
      seq: 4,
      time: 3_000,
      callId: 'same',
      call: { name: 'bash', argsRaw: '{"command":"first"}' },
      callTime: 2_500,
    ...
    const turns = deriveTrajectoryLayout({
      nodes: [{
        ...
        blocks: [{ kind: 'tool-call', callId: 'same', name: 'bash', argsRaw: '{"command":"first"}' }],
      }, settled],
      partial: {
        turn: 1,
        step: 1,
        blocks: [{ kind: 'tool-call', callId: 'same', name: 'bash', argsRaw: '{"command":"second"}' }],
      },
      runningCalls: [running],
      toolLifecycles: [
        { rootCallSeq: 3, turn: 1, step: 1, root: settled },
        { rootCallSeq: 6, turn: 1, step: 1, root: running },
      ],
    })

    const tools = turns[0]?.groups.flatMap(group => group.cells)
      .filter(cell => cell.kind === 'tool') ?? []
    expect(tools).toMatchObject([
      { rootCallSeq: 3, outputDetail: 'done', startedAt: 2_500, timeSeconds: 0.5 },
      { rootCallSeq: 6, startedAt: 6_000, timeSeconds: null },
    ])
  })
