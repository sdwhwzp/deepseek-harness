...
describe('TrajectoryTable', () => {
  ...
  it('an inspect target opens the matching record and acknowledges once', () => {
    ...
    render(
      <TrajectoryTable
        turns={CALL_TURNS}
        {...FOLD_PROPS}
        inspectCall={{ callId: 'call-1', rootCallSeq: null }}
  ...
  it('uses the root lifecycle when an inspect target repeats a call id', () => {
    const onInspectApplied = vi.fn()
    const turns: readonly TrajectoryTurnModel[] = [{
      turn: 1,
      groups: [{
        ...
        cells: [
          {
            index: 1, kind: 'tool', text: 'first call', callId: 'same', rootCallSeq: 3,
            outputDetail: 'first output', timeSeconds: 0.1,
          },
          {
            index: 2, kind: 'tool', text: 'second call', callId: 'same', rootCallSeq: 6,
            outputDetail: 'second output', timeSeconds: 0.1,
          },
        ],
      }],
    }]
    ...
    render(
      <TrajectoryTable
        turns={turns}
        {...FOLD_PROPS}
        inspectCall={{ callId: 'same', rootCallSeq: 6 }}
        onInspectApplied={onInspectApplied}
      />,
    )

    expect(screen.getByRole('row', { name: /first call/ }).getAttribute('aria-selected')).toBe('false')
    expect(screen.getByRole('row', { name: /second call/ }).getAttribute('aria-selected')).toBe('true')
  ...
  it('an unmatched inspect target stays pending without acknowledgement', () => {
    ...
    render(
      <TrajectoryTable
        turns={CALL_TURNS}
        {...FOLD_PROPS}
        inspectCall={{ callId: 'call-missing', rootCallSeq: null }}
        onInspectApplied={onInspectApplied}
      />,
    )
