...
Prompt submission waits for the staged preset to be accepted by the Host, including when the picker roster or Session summary is still loading. Concurrent picker and send actions share one switch request. A refused switch stops submission until the user chooses a preset again; it cannot silently send under the previous preset.
