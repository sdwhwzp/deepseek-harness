...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['slots', 'conversation', 'sessions', 'uiWorkspace'], (scope: ClientContext) => {
    scope.on('conversation/prepare-send', async (sessionId) => {
      const binding = scope.sessions.binding(sessionId)
      if (binding === undefined) return
      if ((scope.sessions.retainInfo(sessionId).getSnapshot().retainedBy.mainView ?? 0) === 0) return
      await seatFor(scope, binding).prepareSend(sessionId)
    })
