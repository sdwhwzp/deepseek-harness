...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'authorizeRequest(request: ConnectionPrincipalRequest): Promise<ConnectionRequestAuthorization>',
        description: 'Apply the Host/Origin fence and accept either a verified deployment principal or an authenticated browser session.',
        parameters: [{ name: 'request', description: 'carrier request facts owned by the HTTP or upgrade route.' }],
        returns: 'the accepted caller identity or a rejection status.',
      },
      {
        signature: 'authenticateRequest(request: ConnectionPrincipalRequest): Promise<AuthenticatedPrincipal | undefined>',
        description: 'Authenticate, validate, detach, and freeze one deployment principal.',
        parameters: [{ name: 'request', description: 'carrier request facts owned by the transport.' }],
      ...
      {
        signature: 'admitRequest(request: ConnectionPrincipalRequest): Promise<PeerAdmission>',
        description: 'Authenticate a carrier request and select its account-specific Peer.',
        parameters: [{ name: 'request', description: 'trusted carrier request facts.' }],
        returns: 'the admitted Peer and principal, or a rejection status.',
      },
      {
        signature: 'principalOfPeer(peer: PeerScope): AuthenticatedPrincipal | undefined',
        description: 'Read the verified identity bound to a Connection-owned Peer.',
        parameters: [{ name: 'peer', description: 'the admitted Peer.' }],
  ...
  },
  {
    ...
    summary: 'Optional Host authentication provider invoked on transport-owned request facts.',
    description: 'Optional Host authentication provider invoked on transport-owned request facts.',
    methods: [
      {
        signature: 'authenticate( request: ConnectionPrincipalRequest, ): AuthenticatedPrincipal | undefined | Promise<AuthenticatedPrincipal | undefined>',
        description: 'Verify the carrier authentication data and return its caller identity.',
        parameters: [{ name: 'request', description: 'transport-owned request facts (headers, peer, method, URL).' }],
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Select the first available account model after login when no provider API key is configured. Authenticated deployments require an administrator.',
        parameters: [],
      ...
      {
        signature: '@Remote(\'workspacePathApplications\') async workspacePathApplications( request: { readonly sessionId: SessionId; readonly path: string }, signal: AbortSignal, ): Promise<readonly SessionWorkspacePathApplication[]>',
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Register globally or in the calling agent scope. Scoped tools shadow globals; duplicates within one layer and the reserved `run_code` name fail.\n\n`parameters` must be object-rooted and free of root composition keywords (assertToolParametersRoot) — a schema that is not reaches the provider verbatim and fails the whole request, not just this tool. A bridge forwarding an external schema normalizes that root before registering.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'ConnectionPrincipalRequest',
    declaration: 'export interface ConnectionPrincipalRequest extends ConnectionTrustRequest {\n    readonly method?: string | undefined;\n    readonly url?: string | undefined;\n}',
  },
  {
    name: 'ConnectionRequestAuthorization',
    declaration: 'export type ConnectionRequestAuthorization = {\n    readonly accepted: true;\n    readonly principal?: AuthenticatedPrincipal;\n} | {\n    readonly accepted: false;\n    readonly status: 401 | 403;\n};',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface TypertGatewayWireStream {\n    readonly open: (endpoint: string, payload: unknown, uplink: AsyncIterable<unknown>, peer: PeerScope | undefined, signal: AbortSignal, principal?: AuthenticatedPrincipal) => Promise<AsyncIterable<unknown>>;\n    readonly failure: (error: unknown) => {\n        readonly code: string;\n        readonly message: string;\n        readonly details: object;\n    };\n}',
  },
  {
