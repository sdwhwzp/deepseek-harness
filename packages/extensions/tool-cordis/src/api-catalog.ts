...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'cachedPredecessorListHints( meta: SessionHeader, inheritedEventCount: SessionLogOffset, ): ProjectionSnapshot | undefined',
        description: 'Read format-compatible predecessor values as zero-I/O listing hints.\n\nA lifecycle-matching title may be stale but remains meaningful after migration. V2-to-V3 also preserves turn/start and human-message times, so that exact edge admits sessionListMetadata. Each row still requires its current projection version and schema; no hint can seed hydration.',
        ...
        returns: 'compatible listing values with `asOfSeq: -1`, or `undefined` for current, newer, unrelated, missing, or incompatible records. The sentinel discards sequences that migration may have remapped.',
      },
      {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  },
  {
    ...
    declaration: 'export type ConnectionRpcHandler = (endpoint: string, payload: unknown, signal: AbortSignal, principal?: AuthenticatedPrincipal) => Promise<ConnectionRpcResult<unknown>>;',
  },
  {
