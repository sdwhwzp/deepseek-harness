...
`ctx.fileReferences.list(agent, query, signal)` returns path-only file and directory candidates for one agent's working directory, deterministically ranked by the provider. Directory mentions render with a trailing `/` so completion can descend another level. Browser consumers call the Session Controller adapter as `ctx.remote.fileReferences.list`; the adapter authorizes its explicit Session id before resolving or resuming the Agent, and the trailing signal cancels a slow autocomplete.
