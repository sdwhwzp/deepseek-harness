...
| `toolMs` | Summed `tool/call` → `tool/result` wall time, paired by the call event seq cited by each result |
...
The fold state holds the eight totals plus in-flight boundaries: `lastTurn` (turn of the last counted `step/end`), `openStep` (the open step's boundary facts, closed by its `assistant/message`), and `pendingCalls` (tool dispatch times by `tool/call` event seq). The wire view is a strict subset — the eight totals — so the persisted-cache state schema extends the view schema with the boundary fields.
...
- Tool time pairs `tool/call` → `tool/result` by the call event seq cited by the result; unresolved calls are dropped at `turn/end`, and a result without a recognized citation contributes no tool time.
