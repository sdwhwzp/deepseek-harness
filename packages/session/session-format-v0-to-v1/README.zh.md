...
用户角色消息与 `turn/start`、`step/start` 上的可选 `principal` 元数据保留所记录的认证提供方、账号 id、用户名与 admin/user 角色。同一套校验适用于收件箱消息以及标题请求内的消息。未预期的身份成员、非法角色，以及出现在 assistant/system 消息上的身份一律拒绝迁移；本就不存在的身份保持不存在。移除历史 turn trigger 时保留 principal。
