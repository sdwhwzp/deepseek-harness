...
经过认证的载体可以把一个已验证 `AuthenticatedPrincipal` 绑定到进程内 Request。`toFetchHandler` 把这个仅 Host 可写的值复制到 `RpcRequest.principal`；`session.prompt` 随后将它快照到新建的 `UserMessage`。浏览器提交的身份外观字段仍只是普通不可信载荷，并会被领域 schema 剥离。该绑定不可变、只属于单条请求，并且绝不序列化进响应。
