...
// A cold session reuses one provider id for two settled calls before a later
// assistant reply. Reopening that session must render both calls and the reply.
...
const SNAPSHOT_DIR = fileURLToPath(new URL('./snapshots/repeated-tool-call-id', import.meta.url))
const UI_EXPECTED = fileURLToPath(new URL('./snapshots/repeated-tool-call-id/ui.expected.md', import.meta.url))
const MODE = webSnapshotMode()
const SEED_ID = 'repeated-tool-call-id-web-e2e'
const REUSED_CALL_ID = CallId('provider-reused-call-id')
...
function repeatedCallIdFixture(): string {
  ...
  session.append('session/title', {
    title: 'Repeated provider call id', messageSeqs: [user.seq], source: { kind: 'fallback' },
  })
  ...
  for (const call of calls) {
    const source = session.append('tool/call', {
      turn: 1, step: 1, callId: REUSED_CALL_ID, name: 'bash', arguments: call.args,
    })
