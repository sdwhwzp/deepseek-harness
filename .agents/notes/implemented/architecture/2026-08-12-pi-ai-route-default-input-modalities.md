...
The harness treats an omitted modality as negative capability. Prompt admission and model selection act on it before any request is built; `read_image` was also gated when this decision shipped. [Tool-result image display on text-only routes](../feature/2026-08-26-tool-result-image-display.md) later removed only the `read_image` gate so the user can see its durable result while a text-only request receives a placeholder. The original diagnostics told the user to select an image-capable model — advice with no reachable referent, because no configuration key could make a hand-declared model image-capable. The route was closed at the metadata, not at the capability: the request converter and every pi-ai wire protocol carry images, and `llm-pi-ai`'s own stream guard is the only thing that would have stopped one.
...
A vision model on a custom provider costs one line, `input: [text, image]`, written in the settings document — or one line at the route when every model it lists takes images. Prompt admission and model selection then admit images on it, and model requests receive pixels instead of placeholders. `read_image` now persists its user-visible result independently of that declaration. A deployment that writes nothing keeps the same reported modalities.

The prompt and model-selection image gates keep their meaning because every modality they read is either recorded by the installed catalog or written by a person. Nothing claims a capability on a deployment's behalf.
...
No keyless snapshot lane exercises a pi-ai route: the snapshot examples drive `dsh-llm-replay`, which declares modalities directly in its configuration, and a pi-ai route needs a live endpoint whose port a static `cordis.yml` cannot name. Prompt and model-selection admission are covered there through that provider. `examples/acp-agent/image-text-route.cordis.snapshot.yml` separately pins route-independent `read_image` persistence under a text-only route and the request projection that keeps pixels out of that model.
