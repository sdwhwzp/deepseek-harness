...
The harness treats an omitted modality as negative capability: model selection refuses to switch into a session that already holds images, and prompt admission refuses a new image. Their diagnostics tell the user to select an image-capable model — advice with no reachable referent, because no configuration key could make a hand-declared model image-capable. [`read_image` persists its user-visible result independently of that declaration](../feature/2026-08-26-tool-result-image-display.md), while request projection substitutes a placeholder for a text-only model. The route was closed at the metadata, not at the capability: the request converter and every pi-ai wire protocol carry images, and `llm-pi-ai`'s own stream check is the only thing that would have stopped one.
...
A vision model on a custom provider costs one line, `input: [text, image]`, written in the settings document — or one line at the route when every model it lists takes images. Prompt admission and model selection then accept images on it, and model requests receive pixels instead of placeholders. `read_image` persists its user-visible result independently of the declaration. A deployment that writes nothing keeps the same reported modalities.

The prompt-admission and model-selection checks keep their meaning because every modality they read is either recorded by the installed catalog or written by a person. Nothing claims a capability on a deployment's behalf.
...
No keyless snapshot lane exercises a pi-ai route: the recorded-session corpus drives `dsh-llm-replay`, which declares modalities directly in its configuration, and a pi-ai route needs a live endpoint whose port a static `cordis.yml` cannot name. [`read-image`](../../../../snapshots/session/read-image/) covers image-capable delivery, while [`read-image-text-route`](../../../../snapshots/session/read-image-text-route/) covers route-independent persistence and the placeholder sent to a text-only model.
