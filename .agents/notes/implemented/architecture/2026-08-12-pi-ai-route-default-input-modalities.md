...
The harness treats an omitted modality as negative capability. Model selection acts on it for image-bearing history, prompt admission acts on it for user images, and request projection uses it to choose pixels or placeholders for durable images, including `read_image` Tool results. Their diagnostics tell the user to select an image-capable model — advice with no reachable referent when no configuration key can make a hand-declared model image-capable. The route is closed at the metadata, not at the protocol capability: the request converter and every pi-ai wire protocol carry images, and `llm-pi-ai`'s stream guard rejects a request that contradicts the declaration.
...
**Undeclared means `[text]`, and that is the absence of a declaration rather than a guess at the endpoint.** Nothing can interrogate a gateway for its modalities because no OpenAI-compatible listing endpoint reports them. The only safe floor is the modality every supported protocol certainly carries. Under-claiming refuses a user image before it is attached, names the model, and has a documented configuration remedy; a durable Tool-result image remains visible but projects to a placeholder. Over-claiming admits and persists a user image before the provider can reject it. Later requests to that same incorrectly declared route encounter the image again, although selecting a text-only model recovers because request assembly projects durable images to placeholders.
...
A vision model on a custom provider costs one line, `input: [text, image]`, written in the settings document — or one line at the route when every model it lists takes images. Model selection and prompt admission then accept its image capability, and request projection sends it the pixels from durable image content such as `read_image` results. A deployment that writes nothing keeps the text-only declaration, so no existing route changes what it reports.
...
No keyless snapshot lane exercises a pi-ai route: the recorded-session corpus drives `dsh-llm-replay`, which declares modalities directly in its configuration, and a pi-ai route needs a live endpoint whose port a static `cordis.yml` cannot name. [`read-image`](../../../../snapshots/session/read-image/) covers image-capable request delivery, while [`read-image-text-route`](../../../../snapshots/session/read-image-text-route/) covers a durable Tool image becoming a text-only request placeholder. The adapter tests cover what pi-ai reports; the snapshots cover how shared request projection consumes that report.
