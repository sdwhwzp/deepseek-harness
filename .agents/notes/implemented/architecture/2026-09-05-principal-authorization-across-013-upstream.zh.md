...
[工具卡片图片决策](../../archived/feature/2026-08-20-tool-card-image-results.md)继续负责专用卡片的派生和折叠展示。本决策仅取代其通用图片限制：独立的调用树槽位保持每个槽位只有一个声明所有者，无需在 owner props 中传递 React 渲染回调。共享命令排序使用 `rankByName`。
...
**原生文件交付操作保留请求身份。** 交付路由在读取当前查看的 Session 或解析其声明路径前，检查传输层验证的身份。默认应用打开和文件管理器定位均通过 Gateway 传递该身份及准确的 Session id。Session 控制器在交给原生适配器时再次检查访问权限，避免直接调用服务丢失身份或绕过授权撤销。
