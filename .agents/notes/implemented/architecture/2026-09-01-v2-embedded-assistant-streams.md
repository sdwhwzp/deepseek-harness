...
Chat retains already materialized Assistant and process-control nodes with hidden visibility when retirement removes their only visible evidence. Returning `null` would violate the Conversation node identity rule before a retry notification can arrive. The [attempt-recovery contract](../../../../packages/client/ui-chat/README.md#attempt-recovery) keeps that transient empty state renderable without adding a durable message or depending on a later frame to repair it.
