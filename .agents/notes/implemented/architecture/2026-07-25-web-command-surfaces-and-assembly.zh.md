...
- 候选项合成会通过当前 client locale 翻译八个已知 Web host 命令。`image` 和 `read-image` 仅在对应工具可用时出现；未知 host 命令保留其 wire 说明，client 贡献项自行管理本地化文案。切换 locale 无需重新拉取目录。
...
| 在 host 命令 descriptor 中携带本地化字符串 | wire 不知道查看者的 locale；单个 descriptor 字符串会迫使所有 client 使用同一语言 |
...
- client 侧本地化让一份命令目录可以跨 locale 切换继续使用；代价是每个新上架的 host 命令都需要增加 `command` 词典项，扩展命令则保留自己的 descriptor 文本。
