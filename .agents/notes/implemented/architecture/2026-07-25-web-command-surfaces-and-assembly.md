...
- Candidate synthesis translates the eight known Web host commands through the active client locale. The `image` and `read-image` rows remain conditional on their backing tools; unknown host commands preserve their wire descriptions, while client contributions own their localized copy. A locale switch therefore needs no directory refetch.
...
| Localized strings in host command descriptors | The wire has no viewer locale; one descriptor string would force every client to the same language |
...
- Client-side localization keeps one command directory valid across locale changes; the cost is that each newly shipped host command needs a `command` dictionary entry while extension commands retain their own descriptor text.
