...
7. **Predecessor values are listing hints, not fold shortcuts**: Session-list startup remains metadata/cache-only and never opens cold log bodies. The log header is authoritative; a lifecycle-matching checkpoint is a durable-prefix witness that may lag but cannot lead the log. `cachedPredecessorListHints` exposes title plus the edge-specific metadata described below, each checked against its current projection version and schema. Its `asOfSeq: -1` discards sequences that migration may remap. Other rows remain hidden, and `hydratePrepared`/`coldSnapshot` retain strict format identity because historical normalizers can change projection semantics.
...
### V2-to-V3 list metadata

`cachedPredecessorListHints()` also admits `sessionListMetadata` when the stored format is exactly 2 and the listed format is exactly 3. This edge preserves `turn/start` existence and human-message timestamps, so compatible row versions retain blankness and recency. A missing or different format generation still exposes only title. The cache remains a possibly stale durable-prefix hint; neither row can seed hydration, and `asOfSeq: -1` discards remapped predecessor sequence numbers.

Dropping a V2 blank marker makes a configuration-only Session visible under its directory name after an upgrade. The Loader regression records blank and nonblank list output, rejects unrelated lifecycles, unsupported edges, row-version mismatches and invalid values, and proves no cold stat, body read, Session activation, or cache rewrite occurs. Migration tests pin both metadata fields across configuration-only, zero-step, human-input, and plugin-input logs.
...
- **Read every cold log or erase empty Sessions**: listing would acquire body I/O and activation risks, while deletion would alter retained history. Compatible listing hints preserve the existing metadata-only path and durable records.
