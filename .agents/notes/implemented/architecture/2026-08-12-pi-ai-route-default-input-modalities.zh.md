...
Harness 把缺失的模态当作否定能力：模型选择拒绝切换进已含图片的会话，prompt 准入拒绝新图片。它们的诊断让用户去选一个支持图片的模型——这条建议没有可达的指向对象，因为没有任何配置键能让手写模型变得支持图片。[`read_image` 独立于该声明持久化用户可见结果](../feature/2026-08-26-tool-result-image-display.zh.md)，请求投影则为纯文本模型替换成占位文本。这条路封死在元数据上而非能力上：请求转换器和每种 pi-ai 协议格式都能携带图片，`llm-pi-ai` 自身的流检查才是唯一会拦下它的东西。
...
自定义提供方下的视觉模型只需在 settings 文档里写一行 `input: [text, image]`——或者当该路由列出的模型全都接受图片时，在路由上写一行。prompt 准入与模型选择随后接纳它上面的图片，模型请求收到像素而非占位文本。`read_image` 独立于该声明持久化用户可见结果。什么都不写的部署保持相同的模态报告。

prompt 准入与模型选择检查保住了自己的意义，因为它们读到的每一个模态，要么由已安装 catalog 记录，要么由人写下。没有任何环节会替部署宣称一项能力。
...
没有任何无密钥 snapshot 通道会跑 pi-ai 路由：录制会话语料驱动的是 `dsh-llm-replay`，它在自己的配置里直接声明模态，而 pi-ai 路由需要一个真实端点，其端口是静态 `cordis.yml` 无法写出的。[`read-image`](../../../../snapshots/session/read-image/)覆盖支持图片的交付，[`read-image-text-route`](../../../../snapshots/session/read-image-text-route/)覆盖与路由无关的持久化以及发送给纯文本模型的占位文本。
