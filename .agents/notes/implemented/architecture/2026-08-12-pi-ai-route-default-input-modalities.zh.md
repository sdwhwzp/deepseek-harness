...
Harness 把缺失的模态当作否定能力。模型选择对含图片历史依据它行动，prompt 准入对用户图片依据它行动，请求投影则依据它为持久图片选择像素或占位符，其中包括 `read_image` 工具结果。相关诊断让用户去选一个支持图片的模型——当没有配置键能让手写模型变得支持图片时，这条建议没有可达的指向对象。这条路封死在元数据上而非协议能力上：请求转换器和每种 pi-ai 线路协议都能携带图片，`llm-pi-ai` 的流前置检查会拒绝与声明相矛盾的请求。
...
**未声明即 `[text]`，而这是「尚未声明」，不是对端点的猜测。** 没有任何环节能询问网关的模态，因为 OpenAI 兼容列表端点不会报告它们。安全的底线是每个受支持协议都确定携带的模态。少声明会在用户图片附加之前拒绝、点名模型，并给出有文档的配置补救方法；持久工具结果图片仍对用户可见，但会投影为占位符。多声明会先接纳并持久化用户图片，再由提供方拒绝。之后对同一错误声明路由的请求还会再次遇到图片，但选择纯文本模型可以恢复，因为请求组装会把持久图片投影为占位符。
...
自定义提供方下的视觉模型只需在 settings 文档里写一行 `input: [text, image]`——或者当该路由列出的模型全都接受图片时，在路由上写一行。模型选择与 prompt 准入随即接纳其图片能力，请求投影也会把 `read_image` 结果等持久图片内容的像素发送给它。什么都不写的部署保留纯文本声明，因此没有任何既有路由改变它报出的内容。
...
没有任何无密钥 snapshot 通道会跑 pi-ai 路由：录制会话语料驱动的是 `dsh-llm-replay`，它在自己的配置里直接声明模态，而 pi-ai 路由需要一个真实端点，其端口是静态 `cordis.yml` 无法写出的。[`read-image`](../../../../snapshots/session/read-image/)覆盖向支持图片的请求交付像素，[`read-image-text-route`](../../../../snapshots/session/read-image-text-route/)覆盖持久工具图片变成纯文本请求占位符。适配器测试覆盖 pi-ai 报告什么，快照覆盖共享请求投影如何使用该报告。
