...
Harness 把缺失的模态当作否定能力。prompt 准入与模型选择会在构造请求前依据它行动；该决定交付时，`read_image` 也受此门禁约束。后续的[纯文本路由下的工具结果图片显示](../feature/2026-08-26-tool-result-image-display.zh.md)只移除了 `read_image` 门禁，使用户能看到持久结果，而纯文本请求接收占位符。原有诊断让用户去选一个支持图片的模型——这条建议没有可达的指向对象，因为没有任何配置键能让手写模型变得支持图片。这条路封死在元数据上而非能力上：请求转换器和每种 pi-ai 线路协议都能携带图片，`llm-pi-ai` 自身的流前置检查才是唯一会拦下它的东西。
...
自定义提供方下的视觉模型只需在 settings 文档里写一行 `input: [text, image]`——或者当该路由列出的模型全都接受图片时，在路由上写一行。prompt 准入与模型选择随即接纳它上面的图片，模型请求会接收像素而非占位符。`read_image` 现在不依赖该声明也会持久保存用户可见结果。什么都不写的部署仍报告相同模态。

prompt 与模型选择的图片门禁保住了自己的意义，因为它们读到的每一个模态，要么由已安装 catalog 记录，要么由人写下。没有任何环节会替部署宣称一项能力。
...
没有任何无密钥 snapshot 通道会跑 pi-ai 路由：snapshot 示例驱动的是 `dsh-llm-replay`，它在自己的配置里直接声明模态，而 pi-ai 路由需要一个真实端点，其端口是静态 `cordis.yml` 无法写出的。prompt 与模型选择准入已通过该提供方覆盖。`examples/acp-agent/image-text-route.cordis.snapshot.yml` 另行固定纯文本路由下不依赖路由的 `read_image` 持久化，以及阻止像素进入该模型的请求投影。
