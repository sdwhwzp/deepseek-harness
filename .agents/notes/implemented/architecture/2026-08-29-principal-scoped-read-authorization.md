...
`session.follow` authorizes before observing the log and again before each frame, so a revocation prevents the next frame from being published. An opted-in [bounded Assistant batch](../bug-fix/2026-09-13-bounded-assistant-follow-batches.md) is one publication frame and receives one fresh check. `session.control` filters all three maps in its opening baseline and resolves each later Session-addressed frame before yielding it. Authorization is therefore applied to both initial state and live events; filtering only the baseline would leak later global queue, job or projection updates.
