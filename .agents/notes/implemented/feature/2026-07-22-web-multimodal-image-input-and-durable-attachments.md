...
History folding preserves `ImageBlock` in user messages, assistant messages, and nested Tool results. User images align to the trailing edge above their text; assistant images remain in their original content-block position in the leading narration flow; every settled root or nested Tool image group appears below its owning row with leading alignment. All three surfaces use the same conversation-owned renderer, session-authorized loader, and per-session attachment cache. `MessageImage` derives a stable inline box from recorded dimensions, resolves bytes through that loader, uses `object-fit: contain`, and turns a missing or corrupt object into a retryable error control.
...
| `packages/client/ui-conversation`, `packages/client/ui-tool`, and `packages/client/ui-attachment` | Per-session draft images, attachment rail, user, assistant, and Tool-result image controls, authorized history loading, and original preview. |
...
- Client unit tests cover paste and drop, mixed clipboard text, image-only send, draft restoration, ordering, draft/session-scope/application object-URL cleanup, root and nested Tool-result galleries, and a deferred historical read that completes after disposal; the keyless assembled built-client lane (`apps/web/tests/image-display.expected.e2e.ts`, `pnpm run test:web`) covers historical user, assistant, and Tool-result galleries over the authorized attachment route and shared cache, the original-size lightbox, and the composer paste rail.
