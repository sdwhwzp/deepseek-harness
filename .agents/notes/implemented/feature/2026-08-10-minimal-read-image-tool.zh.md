...
- **注册随组合条件挂载，执行独立于模型路由。** 工具只在 `ctx.inject(['attachments'], …)` 作用域内注册。持久结果在每种路由下都对用户可见，共享 LLM 运行时则为纯文本请求把其中的图片投影为确定性占位文本。[工具结果图片展示决策](2026-08-26-tool-result-image-display.zh.md)负责展示与模态的分离。
- **PTC mode 以带外方式转发图像**：嵌套分派返回规范值（仅限本次执行，不含图像块），并延迟提交一条携带信封和图像的 `user` 角色上下文消息，图片仍会到达下一次请求。
- **llm-replay 模型可以声明 `inputModalities`**，因此无密钥快照可以覆盖支持图片的交付和纯文本请求投影。
...
- **无条件注册、缺存储时执行报错**——拒绝；没有附件存储的部署永远无法满足该工具，其 schema 会是常态谎言。模型模态不是执行前提，因为持久结果也服务于用户可见历史。

## 后果

- 工具在纯文本路由上执行成功，而该路由用请求期占位文本表示图片。
- 重复的图片结果会累积请求成本，直到请求投影或压缩将其移除；内容寻址只去重持久字节。
- Web 工具结果卡片通过会话授权的附件 loader 与共享灯箱渲染图片。
