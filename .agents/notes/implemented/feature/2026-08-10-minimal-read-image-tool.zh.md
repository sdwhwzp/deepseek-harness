...
- **注册随组合条件挂载，执行提交持久结果时不依赖模型路由。** 工具只在 `ctx.inject(['attachments'], …)` 作用域内注册。共享 LLM 运行时会在请求组装时向纯文本路由提供确定性的占位符，用户仍可通过[纯文本路由下的工具结果图片显示](2026-08-26-tool-result-image-display.zh.md)所定义的 Web 展示看到结果。
- **Code Mode 以带外方式转发图像**：嵌套分派返回规范值（仅限本次执行，不含图像块），并延迟提交一条携带信封和图像的 `user` 角色上下文消息，图片仍会到达下一次请求。
- **llm-replay 模型可以声明 `inputModalities`**，因此 keyless ACP 快照可以覆盖支持图片的交付和纯文本请求投影。
...
- **无条件注册、缺存储时执行报错**——拒绝；没有附件存储的部署永远无法满足该工具，其 schema 会是常态谎言。模型模态不是执行前提，因为持久结果也服务于用户可见的历史记录。

## 后果

- 工具在纯文本路由上成功执行，而该路由会用请求期占位符表示图片。
- 重复的图片结果会累积请求成本，直到请求投影或压缩将其移除；内容寻址只去重持久字节。
- Web 工具结果卡片通过会话授权的附件 loader 与共享灯箱渲染持久图片像素。
