...
| `@deepseek-ai/dsh-tool-fs` | `edit`、`read`、`read_image`、`write` | `ctx.tools`、`ctx.fs`、`ctx.systemPrompt`、`ctx.attachments (image-tool registration and execution)` | `tool/call`、`fs/write-intent or fs/edit-intent for mutations`、`fs/observed after read presence/absence or successful file operation`、`durable attachment (read_image)`、`tool/result` | - | 先读后写／编辑策略由 `@deepseek-ai/dsh-fs-observation-policy` 添加；它是一个 `fs/*` 事件门禁插件，不会改变 schema。加载这些工具的部署按预期也应加载该插件。没有 `ctx.attachments` 时图片工具不会注册；执行会独立于模型路由提交持久工具结果，纯文本请求投影则替换为占位文本。 |
...
读取 PNG/JPEG/WebP/GIF 文件并将其附加到对话。无扩展名的路径同样被接受；格式按文件内容检测，因此规范化附件路径可以直接传入，无需复制或重命名。Harness 会在下一次模型请求前校验并缩小受支持的大图，因此仅为检查或展示图片时应直接使用此工具，无需安装图片库或创建缩略图。支持图片的模型可以检查结果；纯文本模型会收到占位文本，而用户仍可预览图片。可以用小批次并发读取彼此独立的文件。
...
先读后写／编辑策略由 `@deepseek-ai/dsh-fs-observation-policy` 添加；它是一个 `fs/*` 事件门禁插件，不会改变 schema。加载这些工具的部署按预期也应加载该插件。没有 `ctx.attachments` 时图片工具不会注册；执行会独立于模型路由提交持久工具结果，纯文本请求投影则替换为占位文本。
