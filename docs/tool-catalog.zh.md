...
读取 PNG/JPEG/WebP/GIF 文件并附加到对话中。Harness 会在下一次模型请求前校验并缩小受支持的大图，因此仅为检查或展示图片时应直接使用此工具，无需安装图片库或创建缩略图。支持图片的模型可以检查结果；纯文本模型接收占位符，用户仍可预览图片。可以用小批次并发读取彼此独立的文件。
