...
读取 PNG/JPEG/WebP/GIF 文件并将其附加到对话。无扩展名的路径同样被接受；格式按文件内容检测，因此规范化附件路径可以直接传入，无需复制或重命名。Harness 会在下一次模型请求前校验并缩小受支持的大图，因此仅为检查或展示图片时应直接使用此工具，无需安装图片库或创建缩略图。支持图片的模型可以检查结果；纯文本模型会收到占位符，用户仍可预览图片。可以用小批次并发读取彼此独立的文件。
...
先读后写／编辑策略由 `@deepseek-ai/dsh-fs-observation-policy` 添加；它是一个 `fs/*` 事件门禁插件，不会改变 schema。加载这些工具的部署按预期也应加载该插件。没有 `ctx.attachments` 时图片工具不会注册；执行会在所有模型路由下持久化结果，请求投影控制发送像素还是纯文本占位符。
