...
<a id="ctxprincipalaccess--principalaccessservice-abstract-seam"></a>

### `ctx.principalAccess` — `PrincipalAccessService` (abstract seam)

Service Definition for deployment authorization. Providers validate the principal's source and subject against deployment-owned account data, then return only the requested resource ids that principal may read.
 ...
 */
abstract resolve( principal: AuthenticatedPrincipal, subjects: PrincipalAccessSubjects, signal?: AbortSignal, ): Promise<PrincipalAccessResult>
```

Types: [AuthenticatedPrincipal](core.zh.md)
