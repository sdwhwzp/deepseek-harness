...
| `ctx.requestPrincipal` | `seam` | [`client-connection`](../packages/client/connection) | - | - | - | Connection 传输为每个请求调用的可选 Host 认证服务。部署插件验证账号身份，并将身份传递给 RPC 与 Fetch 路由。 |
