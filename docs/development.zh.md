...
`pre-push` 钩子需要 Node 处于引擎支持范围内（`^22.19 || >=24`）；在更旧的 Node 上，`tsdown` 无法加载 `unrun`。执行 `git push` 前请先运行 `nvm use 22.21.1`（或 `nvm alias default 22.21.1`）。
...
一个部署 profile 可能有不止一个写入方：发布流水线与手工插件更新会改同一份 `package.json`、`pnpm-workspace.yaml` 和 `cordis.patch.yml`，后写的覆盖先写的。改动前先记录钉住的集合——每个 `dependencies` 条目、它的 workspace override，以及每一行 id 定向的 patch 行——事后再确认它们仍在；流水线用旧快照还原时，会无声地回退一次已落地的改动。把钉住的构件（name、version、file、sha256、commit）和不能丢的 patch 行交给另一条流水线的负责人。

**patch 的 `config` 是整体替换，不是合并。** include 插件把 override 的每个键直接赋到目标行上，因此 id 定向的覆盖必须携带该行需要的全部字段。漏掉一个就让它退回 schema 默认值，行为改变却不报任何错——单是丢掉 `currency`，整个计费口径的币种就变了。

**探测部署实际对外提供的入口。** 健康检查写死了协议与端口，而一次无关的改动就能把它们挪走；探测不到健康服务会让部署判定失败，回滚一个本来正常的构建。先试当前协议，再回落到旧协议。瞬间连上却零字节返回是握手失败而非超时，把超时调长只会掩盖它、不会诊断它。

**生产方变了就给缓存打版本。** 按输入身份（文件大小与 mtime、内容哈希）缓存的派生结果，会继续提供旧规则产出的条目，输入没变就永远不会重新派生。当改动改变了生产方统计或存储的内容，给条目盖上生产方版本，版本不符即视为未命中；否则修复到不了任何既有数据，重启也清不掉。

**公开目录不是厂商价目表。** 聚合方的价格、能力与模型 id 带着它自己的汇率换算，滞后于厂商的重新发布，也跟不上改名。用它补齐别处没有覆盖的部分，绝不用它覆盖已核对过的表，并记录每次观测的来源与时间，使一个数字可以被追溯。
