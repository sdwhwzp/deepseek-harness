...
轮次打开时，loop 会为整个轮次固定首个待处理的调用方分组。已认证分组按 `(source, id)` 比较；principal 缺失且来源为 `user` 的消息属于匿名分组，principal 缺失的非用户消息则仍是中性内部上下文。因此，匿名 steering 无法继承已认证请求、工具或计费身份。Loop 会把已认证归属者记录到 `turn/start` 和每个 `step/start`，向 `agent/pre-step` 与 `agent/request` 公开，并传给每次工具执行。工具结果上下文会取得该执行 principal，并保持在竞速到达的其他分组输入之前；其他调用方分组留待后续轮次。
