...
| [`identity/principal-access`](../packages/identity/principal-access/README.zh.md) | 面向传输层已验证 principal 的部署自有 Session 与 Workspace 读取授权 | `ctx.principalAccess` |
...
**工具 occurrence 使用日志身份。** provider `callId` 仍是不透明 payload 数据，并且可以重复。原始 `tool/call` event 的 branded `SessionSeq` 标识一次 occurrence；对应 `tool/result` 首先引用该 seq，嵌套 PTC dispatch 记录则让相同的 `rootCallSeq` 贯穿 agent loop 与工具 runtime。
