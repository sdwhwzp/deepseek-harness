...
| `ctx.requestPrincipal` | `seam` | [`client-connection`](../packages/client/connection) | - | - | - | Optional Host authentication seam the Connection transport consults for every carrier; a deployment plugin (the account gateway) provides it and the verified principal rides every RPC and Fetch route. |
