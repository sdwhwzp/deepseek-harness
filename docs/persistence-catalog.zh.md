 ...
 * root call seq (matching the `tool/ptc-dispatch-start` with the same
 * `subCallId`), the tool `name`
 ...
 * `run_code` call id, its durable root call seq when available, the opaque
 * sub-call id (new calls use `<parent>:ptc:<n>`, numbered in submission
 * order), and the tool `name` with its
