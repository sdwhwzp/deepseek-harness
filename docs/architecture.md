...
At turn opening, the loop fixes the first pending caller group for the full turn. Authenticated groups compare `(source, id)`; a principal-less `user` source is the anonymous group, while principal-less non-user messages remain neutral internal context. Anonymous steering therefore cannot inherit an authenticated request, tool or accounting identity. The loop records an authenticated owner on `turn/start` and each `step/start`, exposes it to `agent/pre-step` and `agent/request`, and supplies it to every tool execution. Tool-result context receives that execution principal and stays ahead of racing foreign-group input; other caller groups remain pending for later turns.
