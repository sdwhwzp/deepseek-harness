...
| [`identity/principal-access`](../packages/identity/principal-access/README.md) | Deployment-owned Session and Workspace read authorization for transport-verified principals | `ctx.principalAccess` |
...
**Tool occurrences use log identity.** A provider `callId` remains opaque payload data and may repeat. The originating `tool/call` event's branded `SessionSeq` identifies one occurrence; its `tool/result` cites that seq first, and nested PTC dispatch records carry the same `rootCallSeq` through the agent loop and tool runtime.
