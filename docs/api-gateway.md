...
The Connection performs the unified trust check for `/api` before the HTTP bridge, then evaluates every registered interceptor's endpoint claim inside the shared FetchHandler. Exactly one claim receives the request, no claim falls back to the existing API Proxy, and overlapping claims return 500 before any interceptor runs. The Typert Gateway claims only two-segment endpoints that have a strict descriptor or active SRC marker. The Connection owns transport, RPC ids, response envelopes, request cancellation, and shared endpoint selection, while the Gateway owns only the Remote data protocol and business dispatch. Replacing the Connection carrier in the future does not require changes to Remote descriptors or the Client programming interface.
